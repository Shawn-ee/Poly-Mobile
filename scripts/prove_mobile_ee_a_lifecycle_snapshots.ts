...
const OUTPUT_PATH = "docs/mobile/harness/cycle-EE-A-lifecycle-snapshots.json";
const DEFAULT_BASE_URL = "http://127.0.0.1:3012";
const PROVIDER_DEPTH_SOURCE = "polymarket-clob-ee-lifecycle-snapshot-proof";
...
async function createProviderBackedBookLineMarket() {
  ...
  return prisma.market.create({
    data: {
      slug: `ee-a-book-lifecycle-spread-${suffix}`,
      title: "Spain vs Japan - Provider Book Spread",
      description: "EE-A backend proof market for selected Book identity lifecycle.",
      status: "LIVE",
      mechanism: "ORDERBOOK",
      visibility: "PUBLIC",
      kind: "ORDERBOOK",
      ...
      externalSlug: `ee-a-spain-japan-spread-2h-${suffix}`,
      externalMarketId: `gamma-ee-a-spain-japan-spread-2h-${suffix}`,
      conditionId: `condition-ee-a-spain-japan-spread-2h-${suffix}`,
        ...
        create: [
          {
            ...
            side: "yes",
            code: "YES",
            slug: `ee-a-spain-spread-yes-${suffix}`,
            displayOrder: 0,
            isActive: true,
            isTradable: true,
            referenceTokenId: `token-ee-a-spain-spread-yes-${suffix}`,
          ...
          },
          {
            ...
            side: "no",
            code: "NO",
            slug: `ee-a-spain-spread-no-${suffix}`,
            displayOrder: 1,
            isActive: true,
            isTradable: true,
            referenceTokenId: `token-ee-a-spain-spread-no-${suffix}`,
...
async function main() {
  if (process.env.NODE_ENV === "production") {
    throw new Error("Refusing to create EE-A mobile lifecycle proof state in production.");
  }
  ...
  const proofUser = await createUser("holiwyn_ee_a_lifecycle_snapshot_user", "10000", true);
  const makerUser = await createUser("holiwyn_ee_a_lifecycle_snapshot_maker", "10000", true);
  const credential = await createApiCredential({
    userId: proofUser.id,
    name: `EE-A-book-lifecycle-${new Date().toISOString()}`,
  ...
  const openOrderRequest = {
    ...
    price: "0.32",
    size: "10",
    clientOrderId: `ee-a-open-${randomUUID()}`,
    contractSide: "YES",
  ...
  const openOrderResult = await submitCanonicalOrder({
    ...
    idempotencyKeyHeader: `ee-a-open-${randomUUID()}`,
  });
  ...
  const fillOrderRequest = {
    ...
    price: "0.44",
    size: "12",
    clientOrderId: `ee-a-fill-${randomUUID()}`,
    contractSide: "YES",
    selection: expectedIdentity,
  };
  const fillOrderResult = await submitCanonicalOrder({
    ...
    idempotencyKeyHeader: `ee-a-fill-${randomUUID()}`,
  });
  ...
  const summary = {
    ...
    proof: "EE-A selected provider-backed Book line selection snapshot preserves provider/source/token fields through order response, portfolio open orders, canceled activity, filled position, and recent trade activity without moneyline fallback.",
    limitation: "No schema migration was added. Positions and recent trades now use the latest matching ApiOrderRequest selection snapshot when one exists, with current market/outcome metadata as the guarded fallback.",
    baseUrl,
