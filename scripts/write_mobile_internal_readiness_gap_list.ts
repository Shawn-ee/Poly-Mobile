...
const blockerRows: Record<string, Omit<GapRow, "priority">> = {
  ...
  s23_local_mvp_device_proof_not_ready: {
    ...
    expectedBehavior: "Committed S23 proofs for Spread filled buy/history, Spread open-order cancel, Spread cashout/sell, Totals filled buy/history, and Team Totals filled buy/history should all pass recently on `SM_S911U1`.",
    affectedFilesRoutes: "`docs/mobile/harness/cycle-XG-full-local-mvp-s23-flow`; `cycle-XH-open-order-cancel-s23-flow`; `cycle-XI-cashout-sell-s23-flow`; `cycle-WF-line-family-s23-proof`; `cycle-WG-team-total-s23-proof`.",
  ...
  temporary_sportsbook_backend_proof_stale_or_missing: {
    pageFunction: "Temporary sportsbook backend proof freshness",
    actualBehavior: "The temporary sportsbook provider/backend proof artifacts are missing, failed, or older than the allowed freshness window.",
    expectedBehavior: "The one-event sportsbook seed and backend fake-token order/Portfolio/history proof should stay fresh while the Local MVP uses the temporary sportsbook bridge.",
    affectedFilesRoutes: "`scripts/seed_the_odds_api_single_event.ts`; `scripts/prove_mobile_the_odds_api_single_event_flow.ts`; `/api/events`; `/api/mobile/events/:slug/live-detail`; `/api/orders`; `/api/portfolio`; `/api/portfolio/history`.",
...
const markdown: string[] = [
  ...
  `- Generic non-soccer World Cup matches excluded: ${readiness.excludedGenericWorldCupMatchEventCount ?? "unknown"}`,
  ...
  `- S23 Google callback: ${readiness.googleS23ConsentExpectedCallback ? `\`${readiness.googleS23ConsentExpectedCallback}\`` : "unknown"}`,
  "",
  "## Open Issues",
  "",
];
