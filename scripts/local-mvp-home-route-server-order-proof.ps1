param(
  ...
  [string]$OutputDir = "docs\mobile\screenshots\cycle-FG-home-route-server-order",
  [string]$HierarchyOutputDir = "docs\mobile\harness\cycle-FG-home-route-server-order"
)
...
$proofUsername = "holiwyn-mobile-fg-$stamp"
$eventProofPath = "docs/mobile/harness/cycle-FG-home-route-server-order/cycle-FG-home-route-server-order-event.json"
...
try {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Tablet Home route-backed server order smoke failed."
    }

    $summaryPath = Join-Path $repoRoot (Join-Path $HierarchyOutputDir "cycle-FG-home-route-server-order-wrapper.json")
    ...
    [ordered]@{
      ...
      tabletProofPath = Join-Path $HierarchyOutputDir "cycle-FG-home-route-server-order-proof.json"
      orderMode = "server"
