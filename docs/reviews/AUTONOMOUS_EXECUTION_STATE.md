...
Current `dev` commit at last update: `97c00e0`
...
- DOC-084: Checkpoint refresh after PR #210, merged as PR #211.
- DOC-085: Checkpoint refresh after PR #211, merged as PR #212.
...
- PR #210: Draft docs-only checkpoint refresh after PR #209. It is superseded by merged PR #211 and PR #212 and should be closed, updated, or reconciled by a maintainer rather than merged as-is.
...
2. Close or reconcile stale draft checkpoint PR #210, which is now superseded by merged PR #211 and PR #212.
3. Optional local-only anonymous route smoke run only when a safe local app instance is available; use the anonymous and mobile viewport checklists, and record evidence without secrets or production data.
4. Continue small display-only public-page UI PRs only if they avoid wallet/funding, order/trading, auth/admin, bot, deployment, package/workflow/script, Prisma, and financial logic and follow `docs/reviews/UI_STATE_TERMINOLOGY_MAP.md` for state copy.
...
6. Package/workflow/public API test-lane promotion remains human-reviewed.
...
- PR #175, PR #176, follow-up docs-only PRs #179-#186, #188, #190, #191, #193, #194, #196, #199, #200, #197, #201, #202, #204, #208, #209, #211, #212, and event-detail state-copy PR #203 are merged. Continue with docs-only state refreshes, screenshot/manual evidence preparation, or focused follow-up tasks only when the scope is clearly safe.
