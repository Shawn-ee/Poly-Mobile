...
- Feature/page: Portfolio History row after a server-backed filled Local MVP order.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. History rows now expose `portfolio-history-retail-row-parity`, render a compact Yes/No pill, shorten provider fixture titles to team abbreviations, and show market type/line as a separate subtitle.
- User interactions supported: user submits the simple swipe-confirm ticket, lands in Portfolio, switches to History, and sees a cleaner Polymarket-like activity row with readable side, outcome, event, amount, and time.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle only changes History row presentation and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8331 -OutputDir docs\mobile\screenshots\cycle-HT-portfolio-history-retail-row-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-HT-portfolio-history-retail-row-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HT-portfolio-history-retail-row-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HT-portfolio-history-retail-row-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: the overall Portfolio header/chart still needs additional P1 visual parity work; this cycle focuses on the post-trade History row.

## Cycle HU - Event Detail Retail Chart Surface

- Feature/page: live Event Detail chart/probability surface in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. `EventDetail` now exposes `event-detail-chart-retail-surface-fit`, hides the live-match info strip plus chart outcome/point/filter controls from the default visible Local MVP surface, keeps chart/provider metadata in accessibility proof labels, and lets the user tap the chart surface to cycle preview points.
- User interactions supported: user opens a route-backed live Event Detail page, sees a cleaner probability chart and selected-market trade prompt, can tap the chart to preview probability movement, scrolls to a line market, opens the simple ticket, swipes up to submit a server-backed fake-token order, and reaches Portfolio History.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; selected market, line, period, provider source/token, chart status, and ticket handoff identity remain preserved.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8336 -OutputDir docs\mobile\screenshots\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass -HierarchyOutputDir docs\mobile\harness\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass\cycle-EY-holiwyn-route-server-mvp-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HU-event-detail-retail-chart-surface-s23-proof-final-pass\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket chart press physics and historical time-range controls remain P1/P2; this cycle removes default visual clutter and keeps the Local MVP path focused on prediction/trade.

## Cycle HV - Portfolio Position Row Retail Density

- Feature/page: Portfolio Positions row after a server-backed filled Local MVP order.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. `Portfolio` now exposes `portfolio-position-retail-density-fit` and tightens the position row icon, title, meta, value, and action sizing so the post-trade position reads more like a compact retail Polymarket row on S23.
- User interactions supported: user submits the simple swipe-confirm ticket, lands in Portfolio, and sees a denser position row with visible value, chance, Cash out, and buy-more action; History remains reachable afterward.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio position/history`; this cycle only changes Positions row presentation and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8337 -OutputDir docs\mobile\screenshots\cycle-HV-portfolio-position-row-retail-density-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-HV-portfolio-position-row-retail-density-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HV-portfolio-position-row-retail-density-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HV-portfolio-position-row-retail-density-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HV-portfolio-position-row-retail-density-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: overall Portfolio header/chart parity remains future P1; this cycle focuses on the filled position row density and action readability.

## Cycle HW - Event Detail Player Props Blank MVP Gate

- Feature/page: Event Detail market tab behavior in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. `EventDetail` now exposes `event-detail-player-props-blank-local-mvp` when the user taps Player Props, documenting that player props are intentionally unavailable for the current soccer MVP.
- User interactions supported: user can tap Player Props, see a blank/unavailable state rather than unbuilt markets, return to Game Lines, choose a line market, open the simple ticket, swipe up to submit, and reach Portfolio History.
- State transitions: unchanged for `event detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; Player Props does not create tickets or orders in this MVP.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8339 -OutputDir docs\mobile\screenshots\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass -HierarchyOutputDir docs\mobile\harness\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass\cycle-EY-holiwyn-route-server-mvp-player-props-blank.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HW-event-detail-player-props-blank-mvp-s23-proof-pass\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: real player props remain out of scope for the current MVP as requested.

## Cycle HX - Ticket Header Retail Readability

- Feature/page: simple Buy/Sell ticket header in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. `TradeTicket` now derives a retail event title from the event's two teams when available, exposes `ticket-header-retail-readable`, and lets the selected line wrap to two lines instead of truncating immediately.
- User interactions supported: user opens a line-market ticket and sees a clearer event/outcome header before entering an amount and swiping to buy; the route-backed ticket/order/Portfolio flow remains unchanged.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only ticket header presentation and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8340 -OutputDir docs\mobile\screenshots\cycle-HX-ticket-header-retail-readability-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-HX-ticket-header-retail-readability-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HX-ticket-header-retail-readability-s23-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HX-ticket-header-retail-readability-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HX-ticket-header-retail-readability-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact team flag artwork is still fixture/provider dependent; long fixture team names can still ellipsize at the far right, but the header now uses matchup-style copy instead of provider slug copy.

## Cycle HY - Ticket Swipe Reference Layout

- Feature/page: simple Buy/Sell ticket amount-entry screen in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `SwipeSubmitControl` still submits only after the upward threshold, now exposes progress-motion proof labels, and translates the handle from gesture progress. `TradeTicket` now uses a clearer dark amount/keypad body with rounded lower corners and a fixed red/pink swipe area.
- User interactions supported: user opens a line-market ticket, sees header/team/event/outcome, amount, `to win`, odds/available balance, `+$25` / `+$50` / Max presets, full keypad, and swipes upward on the fixed bottom area to place the fake-token server-backed order.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only ticket presentation, warning-state visibility, gesture feedback, and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8343 -OutputDir docs\mobile\screenshots\cycle-HY-ticket-swipe-reference-layout-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-HY-ticket-swipe-reference-layout-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\mobile\docs\mobile\screenshots\cycle-HY-ticket-swipe-reference-layout-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\mobile\docs\mobile\screenshots\cycle-HY-ticket-swipe-reference-layout-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\mobile\docs\mobile\harness\cycle-HY-ticket-swipe-reference-layout-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket transition physics and flag artwork remain P2 polish; the S23 ticket-ready screenshot now shows the full keypad above the fixed swipe band without overlap.

## Cycle HZ - Portfolio Ticket Amount Integrity

- Feature/page: server-backed fake-token ticket submit into Portfolio Positions/History in the Local MVP route-backed totals flow.
- Frontend components touched: `scripts/local-mvp-route-server-filled-totals-proof.ps1`, `scripts/local-mvp-route-server-filled-team-total-proof.ps1`, `scripts/smoke.ps1`.
- Important functions/services touched: no runtime app component or backend route changed. The mobile proof wrappers now seed enough deterministic counterparty asks for the reference-style `$75` ticket amount to fully fill; the smoke gate now asserts `Cost 75 USDT` in Portfolio.
- User interactions supported: user enters `$75` with `+$25` and `+$50`, swipes to buy, receives a fully filled server-backed fake-token order, and sees Portfolio display the same `$75` cost/value instead of a partial-fill `27.6 USDT` row.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle fixes the proof/data setup so the filled route reflects the amount the user submitted.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`, `scripts/local-mvp-route-server-filled-totals-proof.ps1`, and `scripts/local-mvp-route-server-filled-team-total-proof.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8345 -OutputDir docs\mobile\screenshots\cycle-HZ-portfolio-ticket-amount-integrity-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-HZ-portfolio-ticket-amount-integrity-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HZ-portfolio-ticket-amount-integrity-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-HZ-portfolio-ticket-amount-integrity-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-HZ-portfolio-ticket-amount-integrity-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: this is still deterministic proof liquidity, not organic user liquidity; it is correct for the Local MVP server-backed fake-token flow.

## Cycle IA - Portfolio History Fill Aggregation

- Feature/page: Portfolio History after a server-backed fake-token line-market buy.
- Frontend components touched: `src/services/portfolioHistoryService.ts`, `src/components/Portfolio.tsx`, `src/types.ts`, `scripts/smoke.ps1`, `src/__tests__/portfolioHistoryService.test.ts`.
- Important functions/services touched: `recentTradesToActivity` now aggregates recent trade fills into one retail activity when they share the same side, market, outcome, line/period/provider selection, and short execution window. If the backend later provides `orderId`, the mapper groups by that durable order identity first.
- User interactions supported: user opens a route-backed totals ticket, enters `$75`, swipes up to buy, and sees one Portfolio History row for the submitted buy instead of separate rows for each maker fill.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> GET /api/portfolio/history -> Portfolio History`; only the mobile history presentation layer groups fill fragments into a single user action row.
- Validation: `npm run typecheck`; `npx vitest run src/__tests__/portfolioHistoryService.test.ts`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8346 -OutputDir docs\mobile\screenshots\cycle-IA-portfolio-history-fill-aggregation-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IA-portfolio-history-fill-aggregation-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IA-portfolio-history-fill-aggregation-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IA-portfolio-history-fill-aggregation-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IA-portfolio-history-fill-aggregation-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: current backend `Trade` rows do not expose a durable user order/execution group ID, so mobile uses a same-selection short execution window fallback until the backend route can return `orderId` or `executionGroupId`.

## Cycle IB - Portfolio Header Retail Density

- Feature/page: Portfolio account header, value chart, range selector, and tabs in the Local MVP post-trade route.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend or service changes. `Portfolio` still consumes the same balance, position value, PnL, value-history, tab, position, order, and activity state.
- User interactions supported: user completes the route-backed `$75` swipe buy, lands in Portfolio, can return to the top of Portfolio, and sees profile/value/cash/chart/range/tabs in a denser S23 layout with funding actions still hidden.
- State transitions: unchanged for `event detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio Positions/History`; this cycle only changes Portfolio header spacing and proof coverage.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8348 -OutputDir docs\mobile\screenshots\cycle-IB-portfolio-header-retail-density-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-IB-portfolio-header-retail-density-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IB-portfolio-header-retail-density-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IB-portfolio-header-retail-density-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IB-portfolio-header-retail-density-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket Portfolio header typography/watermark balance remains P2 polish; this cycle closes phone-width density and proof coverage for the top Portfolio surface.

## Cycle IC - Event Detail Core Lines First

- Feature/page: Event Detail game-line market list in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend/API service changes. `EventDetail` still resolves line tickets through `resolveLineTicketTarget`, preserving backend line-market identity before opening `TradeTicket`.
- User interactions supported: user scrolls through Game Lines and sees half-winner markets as collapsed headers by default, keeping full-game line markets and the route-backed totals ticket path easier to reach.
- State transitions: unchanged for `Event Detail -> totals line selector -> ticket -> POST /api/orders -> Portfolio/History`; this cycle only changes default market-group expansion state and visible separator copy.
- Validation: `npm run typecheck`; `npx vitest run src/__tests__/eventDetailLineTicketService.test.ts`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8349 -OutputDir docs\mobile\screenshots\cycle-IC-event-detail-core-lines-first-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IC-event-detail-core-lines-first-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IC-event-detail-core-lines-first-s23-proof\cycle-EY-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IC-event-detail-core-lines-first-s23-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IC-event-detail-core-lines-first-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket market ordering and animated collapse behavior remain P2; this cycle focuses on reducing default half-market clutter while preserving the MVP ticket path.

## Cycle IJ - Trade Ticket S23 Swipe Reference

- Feature/page: Trade Ticket amount-entry and swipe-to-buy screen in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`, `src/localization/appCopy.ts`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `SwipeSubmitControl` still submits only after the upward threshold; it now moves the handle farther with gesture progress and exposes `swipe-submit-handle-progress-animated` for proof. `TradeTicket` uses a fixed dark amount/keypad body above a fixed red/pink swipe footer.
- User interactions supported: user opens a route-backed line ticket, enters amount with `+$25` and `+$50`, sees large amount, `to win`, odds/balance, keypad, and swipes upward on the pink footer to submit a server-backed fake-token buy.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only mobile presentation, swipe feedback, visible copy, and proof capture.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8353 -OutputDir docs\mobile\screenshots\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IJ-trade-ticket-s23-swipe-reference-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket native transition physics and real flag artwork remain P2 polish; the S23 proof now shows the keypad and amount fully visible above the fixed swipe area.

## Cycle IK - Event Detail Local MVP Tab Rail

- Feature/page: Event Detail market tab rail in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no service changes. `EventDetail` now exposes a default Local MVP market tab rail with only `Game Lines` and `Player Props`; older `Exact Score` and `Halves` branches remain unadvertised in the default user rail.
- User interactions supported: user opens Event Detail, sees only the core prediction tab rail, taps the blank Player Props state, returns to Game Lines, selects the route-backed totals line, opens the simple ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle only changes the visible tab rail and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8354 -OutputDir docs\mobile\screenshots\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof\cycle-EY-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof\cycle-EY-holiwyn-route-server-mvp-player-props-blank.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IK-event-detail-local-mvp-tab-rail-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: `Exact Score` and half-specific markets still exist in legacy component code for future/non-MVP work; they are hidden from the default Local MVP tab rail rather than fully removed.

## Cycle IL - Event Detail Core Lines Order

- Feature/page: Event Detail Game Lines ordering in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `gameLineGroups` now places the full-game team-total group before first-half and second-half winner groups, while `renderGroup`, `resolveLineTicketTarget`, and ticket handoff behavior are unchanged.
- User interactions supported: user opens Event Detail, sees full-game spread/totals/team-total markets before half-specific sections, taps Player Props blank state, returns to Game Lines, opens the route-backed totals ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible market group order and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8356 -OutputDir docs\mobile\screenshots\cycle-IL-event-detail-core-lines-order-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-IL-event-detail-core-lines-order-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IL-event-detail-core-lines-order-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IL-event-detail-core-lines-order-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-player-props-blank.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IL-event-detail-core-lines-order-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IL-event-detail-core-lines-order-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket market ordering across every soccer section and native expand/collapse animation remain P2; this cycle closes the full-game-before-half hierarchy gap for the default MVP flow.

## Cycle IM - Portfolio Settings Action

- Feature/page: Portfolio profile header settings gear in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now tracks `settingsOpen` locally and renders a lightweight account settings sheet when the visible gear is tapped.
- User interactions supported: user completes a fake-token trade, lands on Portfolio, taps the settings gear, sees account/language/trading-mode/funding-disabled rows, closes the sheet, and continues to Portfolio History.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle only makes the visible Portfolio gear functional and proof-covered.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8357 -OutputDir docs\mobile\screenshots\cycle-IM-portfolio-settings-action-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IM-portfolio-settings-action-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IM-portfolio-settings-action-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-settings.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IM-portfolio-settings-action-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IM-portfolio-settings-action-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: this is a Local MVP account sheet, not a full account/settings area; deposits, withdrawals, notifications, social settings, and compliance/location checks remain outside current scope.

## Cycle IN - Portfolio Header Dollar Value

- Feature/page: Portfolio account value header in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now uses `portfolioHeaderMoney` for the top account value, PnL, and cash line while preserving USDT-style amounts in ticket/order/position/history rows.
- User interactions supported: user completes a fake-token trade, lands on Portfolio, sees a Polymarket-like `$10,000` account value header with `$9,925 cash`, opens/closes settings, and continues to Portfolio History.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the Portfolio header's visible value presentation and proof marker.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8358 -OutputDir docs\mobile\screenshots\cycle-IN-portfolio-header-dollar-value-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IN-portfolio-header-dollar-value-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IN-portfolio-header-dollar-value-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IN-portfolio-header-dollar-value-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-settings.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IN-portfolio-header-dollar-value-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IN-portfolio-header-dollar-value-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: the seeded MVP account balance remains the requested default fake-token balance; this cycle changes top Portfolio account display only, not accounting units or server order contracts.

## Cycle IO - Portfolio First Position Visibility

- Feature/page: Portfolio first-screen density after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` keeps the same account value, chart, range selector, tabs, settings sheet, positions, and history state while tightening vertical spacing so the first position row is visible in the first S23 viewport.
- User interactions supported: user completes a fake-token trade, lands on Portfolio, immediately sees the account header plus the traded position row, opens/closes settings, and continues to Portfolio History.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only Portfolio vertical presentation and proof marker placement.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8359 -OutputDir docs\mobile\screenshots\cycle-IO-portfolio-first-position-visibility-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IO-portfolio-first-position-visibility-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IO-portfolio-first-position-visibility-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IO-portfolio-first-position-visibility-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IO-portfolio-first-position-visibility-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Portfolio chart curve and final pixel polish remain P2; this cycle focuses on first-screen post-trade result visibility.

## Cycle IP - Event Detail Sticky Tab Clearance

- Feature/page: Event Detail Game Lines sticky tab rail in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `EventDetail` now gives the sticky market shell bottom clearance and exposes `event-detail-sticky-tab-content-clearance` in the S23 line-market proof.
- User interactions supported: user scrolls Event Detail into Game Lines, sees the sticky Game Lines/Player Props rail without it visually cutting through the next market group, opens the route-backed totals ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only sticky tab spacing and proof expectations.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8360 -OutputDir docs\mobile\screenshots\cycle-IP-event-detail-sticky-tab-clearance-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IP-event-detail-sticky-tab-clearance-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IP-event-detail-sticky-tab-clearance-s23-proof\cycle-EY-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IP-event-detail-sticky-tab-clearance-s23-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IP-event-detail-sticky-tab-clearance-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IP-event-detail-sticky-tab-clearance-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native sticky-header scroll physics remain P2; this cycle closes the visible overlap/clearance issue in the line-market scroll proof.

## Cycle IQ - Ticket Market Icon Identity

- Feature/page: Trade Ticket header identity for non-team line markets in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `TradeTicket` now keeps inferred team flags when available, but uses deliberate market-type icons for non-team tickets such as totals (`%`), spread (`+/-`), and winner/live (`1X2`) instead of a generic color block.
- User interactions supported: user opens a route-backed totals ticket, sees a compact `%` market icon in the header, enters amount, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only ticket header visual identity and proof labels.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8361 -OutputDir docs\mobile\screenshots\cycle-IQ-ticket-market-icon-identity-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IQ-ticket-market-icon-identity-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IQ-ticket-market-icon-identity-s23-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IQ-ticket-market-icon-identity-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IQ-ticket-market-icon-identity-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact production team/league artwork remains P2; this cycle removes the obvious generic placeholder for non-team line tickets.

## Cycle IR - Trade Ticket S23 Swipe Tightening

- Feature/page: Trade Ticket amount-entry and swipe-to-buy screen in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `TradeTicket` now applies phone-height spacing for the dark amount/keypad panel, keeps the red/pink swipe footer separated at the bottom, exposes S23 footer-gap proof labels, increases visible swipe handle feedback without letting the handle disappear, and compacts long provider-style matchup titles such as `Breadth Home vs Breadth Away` into `BHO vs BAW`.
- User interactions supported: user opens a route-backed totals ticket, sees compact header, large amount, `to win`, odds/balance, +$25/+$50/Max presets, full keypad, separate red swipe footer, visible upward handle motion, then completes the upward swipe into Portfolio History.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible ticket layout, title formatting, and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8370 -OutputDir docs\mobile\screenshots\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2 -HierarchyOutputDir docs\mobile\harness\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IR-trade-ticket-s23-swipe-tightening-proof-final2\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket physics/blur and production artwork remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IS - Portfolio History Market Icon Identity

- Feature/page: Portfolio History row identity after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now reuses market/team identity icons for History rows when selection identity exists, so totals rows show `%`, spreads show `+/-`, team flags remain flags, and purely non-market activity can still fall back to action icons.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, enters amount in the ticket, swipes to buy, lands in Portfolio, taps History, and sees the completed trade row with a totals market icon instead of a generic arrow.
- State transitions: unchanged for `Event Detail -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible History row leading icon and proof labels.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8371 -OutputDir docs\mobile\screenshots\cycle-IS-portfolio-history-market-icon-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IS-portfolio-history-market-icon-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IS-portfolio-history-market-icon-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IS-portfolio-history-market-icon-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IS-portfolio-history-market-icon-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact production event/team artwork remains P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IT - Event Detail Line Section Clean Start

- Feature/page: Event Detail Game Lines start area in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `EventDetail` now inserts a dark clean-start separator directly below the sticky Game Lines/Player Props rail and exposes durable S23 proof labels on the Game Lines container.
- User interactions supported: user opens Event Detail, scrolls to Game Lines, sees the first Game Lines market group begin cleanly without a clipped leftover market fragment, taps Player Props blank state, returns to Game Lines, opens the route-backed totals ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Event Detail spacing and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8374 -OutputDir docs\mobile\screenshots\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2 -HierarchyOutputDir docs\mobile\harness\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2\cycle-EY-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IT-event-detail-line-section-clean-start-s23-proof-final2\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native sticky-header scroll physics remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IU - Event Detail Line Header Density

- Feature/page: Event Detail Game Lines market-header density in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `GameLineGroup` now supports a compact `displayTitle`, and the team-total group renders `Team Total Goals` with `BHO goals over 1.5 - Reg. Time` while preserving the full backend/proof title `Full Game Team Total Goals (Reg. Time)` in accessibility metadata.
- User interactions supported: user opens Event Detail, scrolls to Game Lines, sees a shorter team-total market header, can still select line-market outcomes, opens the route-backed totals ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible line-market header text and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; S23 team-total header proof; S23 full-flow totals proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8377 -OutputDir docs\mobile\screenshots\cycle-IU-event-detail-line-header-density-full-flow-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IU-event-detail-line-header-density-full-flow-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IU-event-detail-line-header-density-s23-proof-final\cycle-EZ-holiwyn-route-server-mvp-line-markets.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IU-event-detail-line-header-density-s23-proof-final\cycle-EZ-holiwyn-route-server-mvp-line-markets.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IU-event-detail-line-header-density-full-flow-s23-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IU-event-detail-line-header-density-full-flow-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IU-event-detail-line-header-density-full-flow-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket market naming for every soccer market remains P2; the team-total ticket submit proof run was interrupted by the Samsung launcher after the header was captured, so the full submit regression is covered by the stable totals flow proof.

## Cycle IV - Portfolio Compact Event Identity

- Feature/page: Portfolio Positions and History event identity after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now derives compact provider-breadth event identity from existing title/outcome/selection fields, rendering `BHO 0 - BAW 0` in Positions and `BHO vs BAW` in History while preserving the full provider title and selected market identity in accessibility/proof metadata.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, swipes to buy, lands on Portfolio with a compact event score line, opens settings, and taps History where the activity row uses compact event identity.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio event labels and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8378 -OutputDir docs\mobile\screenshots\cycle-IV-portfolio-compact-event-identity-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IV-portfolio-compact-event-identity-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IV-portfolio-compact-event-identity-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IV-portfolio-compact-event-identity-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IV-portfolio-compact-event-identity-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: provider-breadth compaction is intentionally deterministic for the current route-backed MVP fixture; broader production team/event artwork and exact score/live-state formatting remain P2.

## Cycle IW - Trade Ticket S23 Swipe Layout

- Feature/page: Trade Ticket full-screen amount-entry and swipe-to-buy screen in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`.
- Important functions/services touched: no backend service changes. `SwipeSubmitControl` keeps the existing threshold submission behavior but now gives the handle a visible progress-linked vertical lift while keeping the label centered. `TradeTicket` separates the dark amount/keypad panel from the fixed red/pink swipe footer, adds phone-specific keypad/footer clearance, and keeps rounded lower corners where the dark panel meets the submit area.
- User interactions supported: user opens a route-backed totals ticket, sees the compact header, large amount, `to win`, odds/balance, +$25/+$50/Max presets, full keypad, separated red swipe footer, visible upward handle motion, then completes the upward swipe into Portfolio History.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible ticket layout and swipe motion.
- Validation: `npm run typecheck`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8382 -OutputDir docs\mobile\screenshots\cycle-IW-trade-ticket-s23-swipe-layout-final -HierarchyOutputDir docs\mobile\harness\cycle-IW-trade-ticket-s23-swipe-layout-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IW-trade-ticket-s23-swipe-layout-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IW-trade-ticket-s23-swipe-layout-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IW-trade-ticket-s23-swipe-layout-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IW-trade-ticket-s23-swipe-layout-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket blur/physics and production team artwork remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IX - Portfolio Compact Outcome Label

- Feature/page: Portfolio Positions and History outcome labels after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now uses a visible-only compact outcome label helper that removes period shorthand suffixes such as `RT`, `1H`, and `2H` from row titles while keeping the full `selection.displayLabel`, line, period, provider source, and provider token in hidden proof/accessibility metadata.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, swipes to buy, lands on Portfolio where the position title reads `Yes Over 2.5`, and taps History where the activity row reads `Bought Yes Over 2.5`.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio position/history text and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8383 -OutputDir docs\mobile\screenshots\cycle-IX-portfolio-compact-outcome-label-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IX-portfolio-compact-outcome-label-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IX-portfolio-compact-outcome-label-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IX-portfolio-compact-outcome-label-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IX-portfolio-compact-outcome-label-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket portfolio typography and final pixel polish remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IY - Portfolio Dollar Row Amounts

- Feature/page: Portfolio Positions and History money display after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now formats visible position-row cost, current value, PnL, to-win value, and History row amount with Polymarket-style `$` notation while keeping the same numeric fake-token amounts and full selected market/line/provider identity in hidden proof/accessibility metadata.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, enters `$75`, swipes to buy, lands on Portfolio where the position row reads `Cost $75`, and taps History where the completed trade amount reads `$75`.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio row amount formatting and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8384 -OutputDir docs\mobile\screenshots\cycle-IY-portfolio-dollar-row-amounts-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-IY-portfolio-dollar-row-amounts-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IY-portfolio-dollar-row-amounts-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IY-portfolio-dollar-row-amounts-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IY-portfolio-dollar-row-amounts-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket portfolio typography and final pixel polish remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle IZ - Portfolio Range Watermark S23 Fit

- Feature/page: Portfolio chart range selector and brand watermark row after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now tightens the range selector row for S23 width by reducing side padding, range-pill width, and watermark size, and exposes `portfolio-range-watermark-s23-fit` / `portfolio-brand-watermark-no-clip` proof markers.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, enters `$75`, swipes to buy, lands on Portfolio, scrolls to the top, and sees the range selector plus Holiwyn watermark fit within the phone viewport.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio range/watermark layout and proof gates.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8385 -OutputDir docs\mobile\screenshots\cycle-IZ-portfolio-range-watermark-s23-fit-proof -HierarchyOutputDir docs\mobile\harness\cycle-IZ-portfolio-range-watermark-s23-fit-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IZ-portfolio-range-watermark-s23-fit-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-IZ-portfolio-range-watermark-s23-fit-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-IZ-portfolio-range-watermark-s23-fit-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket watermark opacity and chart physics remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle JA - Portfolio Position Gap S23 Fit

- Feature/page: Portfolio Positions tab landing after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now moves hidden sync/count/latest-order proof nodes below the visible tab content so they remain in the Android hierarchy without creating a large blank gap between the Portfolio tab rail and first position row. The visible position row exposes `portfolio-position-tabs-gap-closed-s23`.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, enters `$75`, swipes to buy, lands on Portfolio, and sees the first position row start immediately below the Positions/Orders/History tab rail.
- State transitions: unchanged for `Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible Portfolio landing layout and proof-node placement.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8387 -OutputDir docs\mobile\screenshots\cycle-JA-portfolio-position-gap-s23-fit-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-JA-portfolio-position-gap-s23-fit-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JA-portfolio-position-gap-s23-fit-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JA-portfolio-position-gap-s23-fit-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JA-portfolio-position-gap-s23-fit-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket row spacing and typography remain P2; no order book, chat, live stats, deposits, withdrawals, or backend order logic were touched.

## Cycle JC - Trade Ticket Swipe Reference Tightening

- Feature/page: Trade Ticket amount-entry and swipe-to-buy screen in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/TradeTicket.tsx`.
- Important functions/services touched: no backend service changes. `SwipeSubmitControl` now uses explicit `SWIPE_SUBMIT_THRESHOLD` and `SWIPE_HANDLE_TRAVEL` constants so the handle visibly follows upward gesture progress without turning the footer into a static tap button. `TradeTicket` keeps the dark amount/keypad body separate from the fixed red/pink submit zone with S23-specific body/footer spacing.
- User interactions supported: user opens Event Detail, selects a line-market outcome, opens the full-screen ticket, enters/presets amount, sees `to win`, odds and balance, sees the full keypad above the red swipe footer, drags upward with visible handle movement, submits only after the threshold, and lands in Portfolio/History through the same server-backed fake-token order path.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible ticket layout and swipe-gesture feedback.
- Validation: `npm run typecheck`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8393 -OutputDir docs\mobile\screenshots\cycle-JC-trade-ticket-swipe-s23-proof-final -HierarchyOutputDir docs\mobile\harness\cycle-JC-trade-ticket-swipe-s23-proof-final`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JC-trade-ticket-swipe-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JC-trade-ticket-swipe-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JC-trade-ticket-swipe-s23-proof-final\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JC-trade-ticket-swipe-s23-proof-final\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket native blur/drag physics and production team artwork remain P2; no order book, chat, live stats, deposits, withdrawals, or backend/order logic were touched.

## Cycle JD - Portfolio History Relative Time

- Feature/page: Portfolio History row timestamp after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now converts existing activity timestamp text into compact relative labels (`Just now`, `min. ago`, `hr. ago`, `d. ago`) while preserving the raw backend/display timestamp in accessibility metadata. The smoke harness now checks the relative-time formatting marker instead of a brittle exact minute value.
- User interactions supported: user opens Event Detail, selects the route-backed totals line, opens the ticket, enters `$75`, swipes to buy, lands on Portfolio, taps History, and sees the activity row time rendered as compact relative time.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio History timestamp formatting and proof markers.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8394 -OutputDir docs\mobile\screenshots\cycle-JD-portfolio-history-relative-time-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JD-portfolio-history-relative-time-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JD-portfolio-history-relative-time-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JD-portfolio-history-relative-time-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JD-portfolio-history-relative-time-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket Portfolio typography and older-history grouping remain P2; no order book, chat, live stats, deposits, withdrawals, or backend/order logic were touched.

## Cycle JE - Event Detail Compact Chart Trade Rail

- Feature/page: Event Detail chart-selected market rail in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `EventDetail` now renders the chart-selected contract handoff as a compact transparent strip instead of a framed card with a `Selected market` eyebrow. The rail still preserves selected market/outcome/line/period/provider identity in accessibility labels and keeps the `Trade` action wired to the same ticket path.
- User interactions supported: user opens Home -> Event Detail, sees a lower-profile selected chart market strip, taps/continues into Game Lines, selects a route-backed line market, opens the ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Home -> Event Detail -> chart/market selector -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Event Detail chart rail layout and proof markers.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8395 -OutputDir docs\mobile\screenshots\cycle-JE-event-detail-compact-chart-trade-rail-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JE-event-detail-compact-chart-trade-rail-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JE-event-detail-compact-chart-trade-rail-s23-proof\cycle-EY-holiwyn-route-server-mvp-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JE-event-detail-compact-chart-trade-rail-s23-proof\cycle-EY-holiwyn-route-server-mvp-top.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JE-event-detail-compact-chart-trade-rail-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket chart physics/gesture tooltip behavior remains P2; no order book, chat, live stats, deposits, withdrawals, or backend/order logic were touched.

## Cycle JF - Event Detail Primary Outcome Colors

- Feature/page: Event Detail primary match-winner outcome colors in the Local MVP route-backed betting flow.
- Frontend components touched: `src/components/EventDetail.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `EventDetail` now applies a presentation-only green/red color pair to the first two primary match-winner outcomes so the header probabilities, chart trace/selected outcome, chart Trade button, bottom primary outcome buttons, and selected-trade rail match the Polymarket-style game page color model. Market/outcome IDs, provider tokens, line selections, and ticket handoff are unchanged.
- User interactions supported: user opens Home -> Event Detail, sees primary game outcomes in green/red, navigates to Game Lines, selects a route-backed line market, opens the ticket, swipes to buy, and reaches Portfolio History.
- State transitions: unchanged for `Home -> Event Detail -> chart/market selector -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Event Detail primary outcome color presentation and proof markers.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8396 -OutputDir docs\mobile\screenshots\cycle-JF-event-detail-primary-outcome-colors-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JF-event-detail-primary-outcome-colors-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JF-event-detail-primary-outcome-colors-s23-proof\cycle-EY-holiwyn-route-server-mvp-top.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JF-event-detail-primary-outcome-colors-s23-proof\cycle-EY-holiwyn-route-server-mvp-top.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JF-event-detail-primary-outcome-colors-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact team color sourcing and production artwork remain P2; no order book, chat, live stats, deposits, withdrawals, or backend/order logic were touched.

## Cycle JG - Portfolio Position To-Win Payout

- Feature/page: Portfolio Positions row payout summary after a route-backed fake-token trade.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service changes. `Portfolio` now derives visible `To win` from filled shares when present, falling back to `cost / entry probability`, so the row displays payout-style economics like Polymarket instead of repeating current/cost value.
- User interactions supported: user opens Event Detail, selects a route-backed line market, opens the ticket, swipes to buy, lands on Portfolio, and sees the position summary show `Cost $75 | To win $163.04 | Entry 46%`.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only visible Portfolio position payout display and proof markers.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8397 -OutputDir docs\mobile\screenshots\cycle-JG-portfolio-position-to-win-payout-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JG-portfolio-position-to-win-payout-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JG-portfolio-position-to-win-payout-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JG-portfolio-position-to-win-payout-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JG-portfolio-position-to-win-payout-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Polymarket Portfolio typography remains P2; no order book, chat, live stats, deposits, withdrawals, or backend/order logic were touched.

## Cycle JH - Trade Ticket S23 Red Swipe Footer

- Feature/page: Trade Ticket amount-entry and swipe-to-buy screen in the Local MVP route-backed trade path.
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The existing fake-token/server-backed order submission remains unchanged.
- User interactions supported: user opens Event Detail, selects a line market, opens the ticket, sees a dark full-screen amount/keypad panel above a fixed red/pink swipe footer, enters amount with presets/keypad, and submits only after an upward threshold swipe.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible ticket layout and gesture affordance.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8398 -OutputDir docs\mobile\screenshots\cycle-JH-trade-ticket-s23-red-swipe-footer-proof -HierarchyOutputDir docs\mobile\harness\cycle-JH-trade-ticket-s23-red-swipe-footer-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JH-trade-ticket-s23-red-swipe-footer-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JH-trade-ticket-s23-red-swipe-footer-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JH-trade-ticket-s23-red-swipe-footer-proof\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JH-trade-ticket-s23-red-swipe-footer-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket blur during the armed swipe remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.

## Cycle JI - Portfolio Result Landing Header

- Feature/page: Portfolio screen immediately after the Local MVP route-backed fake-token order.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The Portfolio still consumes the same balance, positions, orders, activities, and value-history data.
- User interactions supported: after the user places a fake-token order from the ticket, Portfolio lands at the Polymarket-like account/value header instead of jumping halfway down the page and cutting off the avatar/value section.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible scroll landing position and proof marker.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8399 -OutputDir docs\mobile\screenshots\cycle-JI-portfolio-result-landing-header-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JI-portfolio-result-landing-header-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JI-portfolio-result-landing-header-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JI-portfolio-result-landing-header-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JI-portfolio-result-landing-header-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Portfolio pixel polish remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.

## Cycle JJ - Portfolio Position Market Context

- Feature/page: Portfolio Positions row in the Local MVP route-backed fake-token order result.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The Portfolio still consumes the same selection `marketType`, `line`, `displayLabel`, provider token/source, and position fields.
- User interactions supported: after a totals trade fills, the Portfolio position row shows a clearer Polymarket-like title such as `Yes Over 2.5 total goals` instead of only `Yes Over 2.5`, while hidden proof labels still preserve the exact order-time line and provider identity.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible Positions row label and proof marker.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8400 -OutputDir docs\mobile\screenshots\cycle-JJ-portfolio-position-market-context-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JJ-portfolio-position-market-context-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JJ-portfolio-position-market-context-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JJ-portfolio-position-market-context-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JJ-portfolio-position-market-context-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Portfolio row typography remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.

## Cycle JK - Portfolio History Market Context

- Feature/page: Portfolio History row after the Local MVP route-backed fake-token order.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The History row still consumes existing activity selection fields: `marketType`, `line`, `displayLabel`, provider source/token, amount, and timestamp.
- User interactions supported: after a totals trade fills, the Portfolio History row shows a clearer market-aware title such as `Bought Yes Over 2.5 total goals` instead of `Bought Yes Over 2.5`, matching the corrected Positions row while preserving full hidden order identity.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible History row title and proof marker.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8401 -OutputDir docs\mobile\screenshots\cycle-JK-portfolio-history-market-context-s23-proof -HierarchyOutputDir docs\mobile\harness\cycle-JK-portfolio-history-market-context-s23-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JK-portfolio-history-market-context-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JK-portfolio-history-market-context-s23-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JK-portfolio-history-market-context-s23-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact History row typography remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.
...
- Frontend components touched: `src/components/TradeTicket.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The existing fake-token/server-backed order submission remains unchanged.
- User interactions supported: user opens Event Detail, selects a line market, opens the ticket, sees the amount/keypad fully above a fixed red/pink footer, enters an amount, sees the chevron begin near the footer top, and submits only after an upward threshold swipe.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio History`; this cycle changes only the visible ticket layout and gesture affordance.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8404 -OutputDir docs\mobile\screenshots\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2 -HierarchyOutputDir docs\mobile\harness\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-swipe-progress.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2\cycle-EY-holiwyn-route-server-mvp-totals-ticket-ready.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JL-trade-ticket-s23-reference-tightening-proof-final2\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact native Polymarket blur during the armed swipe remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.

## Cycle JM - Portfolio Orders Empty State Route Proof

- Feature/page: Portfolio Orders tab empty state after a route-backed filled fake-token order.
- Frontend components touched: `src/components/Portfolio.tsx`, `scripts/smoke.ps1`.
- Important functions/services touched: no backend service, order service, route, schema, or API contract changes. The Portfolio continues to consume the same `openOrders[]` array from `GET /api/portfolio`.
- User interactions supported: user opens Event Detail, selects a route-backed line market, opens the ticket, enters `$75`, swipes to buy, lands on Portfolio, opens Orders, sees centered `No open orders`, then opens History.
- State transitions: unchanged for `Home -> Event Detail -> Game Lines -> line market -> ticket -> POST /api/orders -> GET /api/portfolio -> Portfolio Orders empty state -> Portfolio History`; this cycle changes only the visible Orders empty-state layout and proof coverage.
- Validation: `npm run typecheck`; PowerShell parser check for `scripts/smoke.ps1`; `git diff --check`; primary Samsung S23 proof `powershell -ExecutionPolicy Bypass -File scripts\local-mvp-route-server-filled-totals-proof.ps1 -Device adb-R3CW20LFMLW-7OpoO6._adb-tls-connect._tcp -Port 8405 -OutputDir docs\mobile\screenshots\cycle-JM-portfolio-orders-empty-state-route-proof -HierarchyOutputDir docs\mobile\harness\cycle-JM-portfolio-orders-empty-state-route-proof`.
- Proof artifacts: `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JM-portfolio-orders-empty-state-route-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-orders.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JM-portfolio-orders-empty-state-route-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-orders.xml`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\screenshots\cycle-JM-portfolio-orders-empty-state-route-proof\cycle-EY-holiwyn-route-server-mvp-portfolio-history.png`; `C:\Users\hecto\Desktop\projects\PolyProj\Poly\docs\mobile\harness\cycle-JM-portfolio-orders-empty-state-route-proof\cycle-EY-local-mvp-route-server-filled-totals-flow-proof.json`.
- Known limitations: exact Portfolio Orders pixel polish remains P2; no order book, chat, live stats, deposits, withdrawals, location check, or backend/order logic were touched.
