...
| BATCH-P1-04 | Google/account runtime convenience | Google/account UI and mobile package preflight scripts exist. Root `package.json` now exposes wrappers that delegate to the mobile preflight, so testers can run the account/auth check from the repo root. Non-strict root proof reaches `accounts.google.com` without printing secrets, but currently reports `Google redirect_uri matches NEXTAUTH_URL callback` as failed. Real Google consent still requires configured Google Cloud redirect URI/NEXTAUTH_URL alignment. | Tester docs should call the root wrapper or the mobile script. The wrapper must not print Google secrets. Strict mode should pass before claiming real Google consent readiness. | P1 improved, still auth-config-gated | `package.json`; `src/__tests__/mobile.manual-testing-env.contract.test.ts`; `docs/mobile/audits/BATCH_INTERNAL_READINESS_HARNESS.md`; `mobile/scripts/google-auth-runtime-preflight.ps1`; `mobile/package.json`; local auth env. | `npm run mobile:google-auth-runtime-preflight` route proof; strict S23 callback proof after aligning Google Cloud authorized redirect URI with `NEXTAUTH_URL/api/auth/google/callback`. | No for fake-token trade path; yes for real Google consent testing. |
| BATCH-P1-05 | Bot/runtime mode clarity | No continuous bot is intentionally running. Existing MVP proof uses one-shot liquidity/credential/counterparty setup. Provider MM readiness remains false because provider books are unavailable/closed. | Final report should clearly state bot is not continuous and avoid recommending local-MM seeding until a usable provider book exists. | P1 improved | package scripts `reference:snapshot-watch`, `bot:e2e:*`, `test:reference-bot-initialization`, `poly:internal-exchange-readiness`. | Process health plus `providerBooksUnavailableOrClosed=true` in batch summary. | No for current fixture-line proof; yes for continuous provider-backed local-MM testing. |
