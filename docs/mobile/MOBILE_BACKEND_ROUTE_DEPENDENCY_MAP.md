...
| Durable local runtime heartbeats | `/api/internal/live-runtime/status`; `npm run mobile:one-event-phase-audit`; `npm run mobile:live-runtime-completion-audit` | GET plus local audit commands | Local/dev only; no provider key and no mobile auth | None | `runtimeHeartbeats.checked/durable/source/records/allExpectedServicesRecorded/quotaSpendingHeartbeatRunning/installedOsService`; each record includes service key/name/kind, status, pid, running, continuous, quota-use, installed-service truth, state path, startedAt, heartbeatAt | Adds `RuntimeServiceHeartbeat`; mirrors local supervisor/result-poller process-state files and OS pid checks into durable DB rows. | No provider data and no fake runtime data. The route records the observed local process state only and spends no quota. | Still not an installed production service. Production needs service ownership, heartbeats emitted by the worker itself, alerting, and multi-event queue state. |
...
| Active settlement decision status | `/api/internal/live-runtime/status` | GET | Local/dev only; no provider key and no mobile auth | None | `settlementDecision.checked`, `pass`, `providerQuotaUsed`, `activeMarketStatus`, `activeEventStatus`, `executionEligibleNow`, `operatorDecision`, `blockers`, `marketMustBeClosed`, `exactConfirmationRequiredKnown`, `activeMarketExecutionAttempted`, `disposableCloneSettlementProven`, `supervisorApprovedSettlementWaitProven`, and `nextSafeAction` | Reads local active-event settlement readiness proof artifact; no DB writes or schema changes in this route | Uses existing redacted settlement readiness evidence only; does not execute settlement and does not expose confirmation phrase. | Production should use durable result/approval/settlement records and authenticated operator controls instead of local proof JSON. |
