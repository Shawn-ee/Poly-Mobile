...
| Google auth runtime readiness batch | `/api/auth/google/start`; batch wrapper `npm run mobile:internal-readiness-batch` | `GET` route preflight | None | Query params: `returnTo=/portfolio`, `mobileReturnTo=<configured mobile return URL>` | Redacted summary fields `googleAuthRuntimeReady`, `googleAuthFailedChecks`, batch P1 `google_redirect_uri_mismatch` or `google_auth_runtime_preflight_has_warnings` | OAuth state cookies only during preflight; no user/token records should be created | Non-strict preflight records warnings without blocking Local MVP fake-token trading | Real Google consent requires `NEXTAUTH_URL/api/auth/google/callback` to exactly match a Google Cloud authorized redirect URI and be reachable from S23. |
