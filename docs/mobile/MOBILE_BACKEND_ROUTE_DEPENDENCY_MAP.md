...
| Mobile/runtime feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/runtime | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Local operator controls boundary | `/api/internal/live-runtime/status`; source evidence from `/api/internal/live-runtime/result-review` and `/api/internal/live-runtime/settlement-queue` inside phase audit | `GET` | Local/dev only; route returns 404 in production or when internal status is disabled; no public mobile auth and no provider key | Optional `phaseAuditInProgress=1` for audit bootstrap | `operatorControlBoundary.mode=local_dev_guarded_operator_controls`, `statusProjectionReadOnly`, `noProviderQuota`, `authenticatedControls.*`, `localControls.resultReviewRoute`, `localControls.settlementQueueRoute`, `localControls.settlementApprovalRoute`, `localControls.settlementExecutionRoute`, `localControls.approvedSchedulerCommand`, `executionSafety.*`, `productionBlockers`, `requiredBeforeProduction`, `p0`, `p1` | Reads existing local proof artifacts plus `OfficialResultReview`, `OperatorAuditEvent`, `CanonicalEvent`, `Market`, and `Event` evidence through phase-audit snapshots; no schema change and no writes from status route | None. The status route projects existing local guarded operator evidence and spends no provider quota. | Local guarded operator controls exist. Production still needs a dedicated settlement-operator role model, production operator UI, installed official-result polling, and service ownership. |
| Current-event result review and settlement queue | `/api/internal/live-runtime/result-review`; `/api/internal/live-runtime/settlement-queue`; projected by `/api/internal/live-runtime/status` | `GET` | Authenticated local admin; dev/internal only; no public mobile route | None | For an unresolved current event: `status=awaiting_result`, `runtimeTruth.settlementEvidenceRequired=false`, `awaitingFinalResult=true`, `executionDecision.operatorDecision=awaiting_final_result`, queue `itemCount=0`, `excludedHistoricalReviewCount`, zero P0 | Reads `Event.externalEventId`, event-owned `Market` rows, `CanonicalEvent`, and `OfficialResultReview`; no writes and no provider calls | None. Historical rows are filtered, not substituted. | Live official-result polling is opt-in/quota-capped and not an installed service. Matching final evidence is required before approval/execution state can become ready. |
...
| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| One-event lifecycle matrix | Local command `npm run mobile:one-event-lifecycle-matrix`; proof artifacts; local DB reads | Local command only | Local developer env; `DATABASE_URL` is loaded from process env, `DOTENV_CONFIG_PATH`, or local `.env` candidates | Optional `eventSlug` and output path | Open/paused/closed/settled proof booleans, current event status, current listed sportsbook market statuses, active-event settlement execution truth, and P0/P1/P2 gaps | Reads existing `Event` and listed sportsbook `Market` rows. No schema change. | None. Missing DB env or missing lifecycle proof artifacts fails the matrix instead of fabricating state. | Installed unattended lifecycle service ownership remains P1; production settlement operator UI/automation remains P1/P2. |

## Cycle ZX - Current Runtime Operator Handoff

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Internal tester operator snapshot | `/api/health`, `/api/internal/live-runtime/status?phaseAuditInProgress=1`, quote route referenced by status, local runtime proof artifacts | GET plus local read-only commands | Local development runtime only; no provider key read by status/snapshot commands | None | Backend health, selected Argentina vs. England event/market/outcome identity, cached/live readiness split, current supervisor/result-poller process state, quota-spending-loop state, mobile route freshness status, tester checklist, and settlement guard state | Reads current `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, durable runtime/proof rows surfaced through status, and `.runtime` process state. No schema change. | None. Cached tester readiness is reported separately from live odds freshness instead of fabricating a fresh provider state. | Installed unattended service ownership remains P1. Live mobile odds freshness requires explicit provider refresh. Production official-result auto-settlement remains P1. |

## Cycle ZY - Live Odds Freshness Pulse

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Bounded live odds refresh for internal tester event | The Odds API `/v4/sports`, `/v4/sports/:sport/events`, `/v4/sports/:sport/events/:eventId/markets`, `/v4/sports/:sport/events/:eventId/odds`; local `/api/health`, `/api/events`, `/api/mobile/events/:slug/live-detail`, `/api/markets/:marketId/quote`, `/api/orders`, `/api/portfolio`, `/api/portfolio/history`, `/api/internal/live-runtime/status` | Provider GETs plus local GET/POST proof routes | Provider key loaded from local process env or ignored `.runtime/secrets/the-odds-api-key.txt`; local fake-token API/session for order and portfolio proof | Provider query remains one event, selected markets `h2h`, `spreads`, `totals`, `h2h_3_way`, `alternate_spreads`, `alternate_totals`, decimal odds, ISO dates. Orders preserve selected market/outcome/line identity. | Quota headers, selected event/market/outcome identity, stale-before-refresh and ready-after-refresh lifecycle, shifted maker bid/ask, filled buy/sell, portfolio/history state, and readiness flags `liveOddsReadyRightNow=true`/`providerSnapshotFresh=true` | Existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `ProviderRefreshRun`, `MarketMakerQuoteRun`, `Order`, `Fill`, `Position`, and `UserBalance`. No schema change. | None. Live provider response refreshed stored provider snapshots; cached mode remains separate and no frontend-only odds were invented. | Installed unattended provider refresh remains P1. Production quota dashboard, multi-event polling, and official-result auto-settlement remain P1/P2. |

## Cycle ZAU - Cached Internal Readiness After S23 Cashout

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Cached internal tester trading readiness | `/api/health`, `/api/internal/live-runtime/status?phaseAuditInProgress=1`, `/api/markets/:marketId/quote`, `/api/events`, `/api/mobile/events/:slug/live-detail`, `/api/orders`, `/api/portfolio`, `/api/portfolio/history`; local commands `mobile:internal-tester-operator-snapshot`, `mobile:internal-tester-readiness-gate`, and `mobile:one-event-live-maker-seed` | GET/POST plus local proof commands | Local/internal status route and fake-token mobile session; no provider key required for cached readiness | Quote uses selected `marketId` and `outcomeId`; orders preserve selected market/outcome/side/size/price; readiness commands accept optional base URL/summary paths | Selected Argentina vs. England event/market/outcome, maker bid/ask, cached/internal/live readiness split, recommended operator action, no-quota loop state, manual trading checklist, P0/P1/P2 gaps, and S23 cashout proof links | Reads existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `Order`, `Fill`, `Position`, `UserBalance`, `ProviderRefreshRun`, `MarketMakerQuoteRun`, `RuntimeServiceHeartbeat`, and `RuntimeServiceRun` evidence. No schema change. | None. Cached trading readiness is accepted from durable backend/maker/order proof; it does not invent fresh provider odds. | Live mobile odds freshness still requires explicit quota-gated provider refresh. Installed unattended service ownership and production official-result auto-settlement remain P1. |

## Cycle ZAW - Active Settlement Readiness Env And Evidence

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Active event settlement readiness report | Local commands `mobile:one-event-result-ingestion-audit-event-proof`, `mobile:one-event-settlement-audit-event-proof`, `mobile:one-event-settlement-approval-audit-event-proof`, `mobile:one-event-result-review-trail`, and `mobile:one-event-active-settlement-readiness` | Local Node commands; no HTTP route change and no provider call | Local developer/runtime only; refuses production. No provider key and no mobile auth. DB env is loaded from local `.env`/`DOTENV_CONFIG_PATH`. | Event slug, optional selected market/outcome override, output path | Active market status, selected outcome, approval event id/digest, exact-confirmation-known truth, execution eligibility, blocker list, active-execution-attempted=false, no-quota truth, P0/P1/P2 gaps | Reads `Market`, linked `Event`, and `CanonicalEvent` rows. Proof commands write canonical `provider.result.ingested`, `settlement.trusted_result.preflight`, and `settlement.trusted_result.approved` events. No schema change. | Uses already-written provider-shaped trusted result fixture and redacted proof summaries. It does not fabricate provider odds, execute settlement, or spend quota. | Active tester settlement still requires `CLOSED` market status plus exact confirmation. Installed official-result polling, production operator UI, and multi-event settlement queue remain P1/P2. |

## Cycle ZAY - Graceful Runtime Worker Stop

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Local supervisor/result-poller graceful stop | Local commands `mobile:one-event-live-supervisor:process`, `mobile:one-event-live-supervisor:stop`, `mobile:one-event-result-poller:process`, `mobile:one-event-result-poller:stop`, and `mobile:internal-tester-readiness-gate`; status route `/api/internal/live-runtime/status?phaseAuditInProgress=1` | Local PowerShell commands plus local GET status during proof | Local developer/runtime only; no provider key, no mobile auth, no production route exposure | Process managers write `.runtime/.../stop-request.json` with `requestedAt`, `pid`, and reason. Workers read the stop request between cycles and during stop-aware sleep. | `operation.graceful=true`, `operation.result=stopped`, worker `pass=true`, current process state, durable runtime run/heartbeat truth, readiness gate P0/P1/P2 gaps | Existing `RuntimeServiceHeartbeat` and `RuntimeServiceRun` rows. No schema change. Status route reads those rows and local `.runtime` process state. | None. If a worker ignores the stop request, the manager falls back to force stop and reports `operation.result=force_stopped`. | Installed unattended service ownership remains P1. This improves local foreground/background worker shutdown; it is not a production daemon. |

## Cycle ZAZ - Tester Readiness Next Action Clarity

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Internal tester readiness gate next-action summary | Local command `mobile:internal-tester-readiness-gate`; upstream `mobile:live-runtime-audit-gate`; upstream `mobile:internal-tester-operator-snapshot`; status route `/api/internal/live-runtime/status?phaseAuditInProgress=1` | Local read-only commands plus local GET status through upstream snapshot | Local developer/runtime only; no provider key, no mobile auth, no production route exposure | Optional summary path only. No provider refresh, loop start, order placement, or settlement execution. | `testerReady.nextAction`, `testerReady.operatorNextAction`, `testerReady.runtimeNextAction`, cached/live readiness flags, cached/live action command metadata, manual checklist, and P0/P1/P2 gaps | No new schema. Reads existing status/snapshot fields and proof artifacts only. | None. Cached readiness can pass from existing backend/maker/order proof without inventing fresh live odds. | Installed unattended service ownership remains P1. Live mobile odds freshness still requires explicit provider-secret refresh when real-time mobile odds are required. |

## Cycle ZBA - No-Quota Live Runtime Readiness Refresh

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| No-quota one-event runtime readiness refresh | `/api/health`; `/api/internal/live-runtime/status?phaseAuditInProgress=1`; `/api/markets/:marketId/quote`; local commands `mobile:one-event-runtime-status`, `mobile:one-event-phase-audit`, `mobile:live-runtime-completion-audit`, and `mobile:internal-tester-readiness-gate` | GET plus local read-only commands | Local developer/runtime only; no provider key, no mobile auth, no production route exposure | No request body. Commands may accept optional summary paths but no provider refresh or order payload is sent. | Selected Argentina vs. England event/market/outcome identity, backend health, quote bid/ask, cached/live readiness split, current supervisor/result-poller loop state, quota-spending-loop state, manual tester checklist, P0/P1/P2 gaps, and operator next action. | Reads existing `Event`, `Market`, `Outcome`, `Order`, `ReferenceQuoteSnapshot`, `ProviderRefreshRun`, `MarketMakerQuoteRun`, `RuntimeServiceHeartbeat`, `RuntimeServiceRun`, `CanonicalEvent`, and `OfficialResultReview` evidence. No schema change. | None. Timestamp-only regenerated proof JSON was not retained; cached trading readiness is accepted only from existing backend/maker/order/S23 evidence. | Installed unattended provider/maker/lifecycle service ownership and production official-result auto-settlement remain P1. Live mobile odds freshness requires the explicit key-gated provider refresh command. |

## Cycle ZBB - Warm No-Quota Runtime Start

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Warm cached internal tester runtime | `/api/internal/live-runtime/status?phaseAuditInProgress=1`; `/api/health`; `/api/markets/:marketId/quote`; local commands `mobile:internal-tester-runtime:cached-start`, result-ingestion/preflight/approval proof commands, and `mobile:live-runtime-audit-gate` | GET plus local runtime/proof commands | Local developer/runtime only; no provider key, no mobile auth, no production route exposure | Runtime manager starts cached supervisor/result-poller with no provider refresh; settlement proof commands write canonical audit evidence but do not execute settlement. | `currentRuntimeState.mode=warm_no_quota_runtime`, `cachedTesterReadyRightNow=true`, `liveOddsReadyRightNow=false`, `supervisorRunning=true`, `resultPollerRunning=true`, `quotaSpendingLoopRunning=false`, settlement approval/preflight truth, and empty P0 gaps. | Reads/writes existing `RuntimeServiceHeartbeat`, `RuntimeServiceRun`, `CanonicalEvent`, `OfficialResultReview`, `MarketMakerQuoteRun`, `ReferenceQuoteSnapshot`, `Market`, and `Outcome` evidence. No schema change. | None. Cached readiness uses stored provider/maker evidence and real local process state; it does not fabricate fresh provider odds. | External Expo listener was reused and remains unverified for server-mode; use the explicit replace-Expo manager path if the phone shows stale behavior. Installed service ownership and production official-result auto-settlement remain P1. |

## Cycle S23CASHOUTMAX - Close-Position Max S23 Proof

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Portfolio close-position ticket Max behavior | `/api/portfolio`; `/api/portfolio/cash-out/estimate`; `/api/markets/:marketId/quote`; `/api/orders` | GET, GET, GET, POST | Existing local fake-token mobile credential; no provider key | Cashout estimate uses owned position identity. Sell submit must use `side=SELL`, owned `marketId`, owned `outcomeId`, selected share quantity, and current sell/bid price. | Owned position shares, `sourcePositionId`, close-position available shares, sell price, estimated proceeds, and sell-mode ticket markers. | Reads/writes existing `Position`, `Order`, `Fill`, `Trade`, `UserBalance`, `Market`, and `Outcome`. No schema change. | None. S23 proof used a real local fake-token position and backend quote/portfolio state. | No P0 missing support for current internal tester cashout path. Dedicated richer cashout preview UX remains P1/P2 polish. |

## Cycle ZBY - Source-Aware Internal Exchange Readiness

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Sportsbook-backed internal exchange readiness | Local command `npm run mobile:internal-exchange-readiness`; source-aware checker `scripts/check_poly_internal_exchange_readiness.ts` | Local DB/read-only command | Local developer env only; loads `DATABASE_URL` from process env, `DOTENV_CONFIG_PATH`, or local `.env` candidates. No provider key and no mobile auth. | CLI args select `referenceSource=sportsbook-odds`, `allowCachedProviderSnapshot=true`, and `requireBotSeeded=false`. | `readyForInternalMobileExchange`, provider source, mobile-visible event count, snapshot-ready count, local-MM-ready count, selected market samples, open local order backing, blockers, and next actions. | Reads existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, and open `Order` rows. No schema change. | Cached sportsbook mode accepts provider-shaped snapshots plus visible local maker orders for internal tester readiness. It does not claim fresh live odds. | Polymarket provider parity remains separate through `npm run poly:internal-exchange-readiness` and still fails until attach-ready Polymarket markets/snapshots exist. Fresh live mobile odds still require explicit quota-gated provider refresh. |

## Cycle ZCC - Cashout Max Runtime Alignment

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body / params | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Home/Event Detail mobile market list | `/api/events?includeMobileMarkets=1`; `/api/mobile/events/:slug/live-detail`; `/api/events/:slug`; `/api/events/:slug/markets` | GET | Public/mobile read route; no provider key | Event slug and mobile market flags | Event identity, `markets`, `market.status`, `market.id`, `outcome.id`, line/period/market type, bid/ask/price, and source metadata | `Event`, `Market`, `Outcome`, local orderbook/quote snapshots | None. Backend and mobile now filter closed/resolved/suspended markets instead of showing stale provider rows. | No P0 missing support for current local MVP. Future provider freshness still requires explicit Odds API refresh. |
| Close-position cashout ticket | `/api/portfolio`; `/api/portfolio/cash-out/estimate`; `/api/markets/:marketId/quote`; `/api/orders`; `/api/portfolio/history` | GET, GET, GET, POST, GET | Local fake-token mobile API credential | Cashout estimate uses `marketId` and `outcomeId`; SELL order sends owned `marketId`, owned `outcomeId`, `side=SELL`, share quantity, price, and ticket selection identity | Owned position shares, optional estimate quantity, sell/bid price, estimated proceeds, order result, updated position/history rows | `Position`, `Order`, `Trade`, `Fill`, `UserBalance`, `Market`, `Outcome` | None. Mobile caps sellable shares to visible owned shares and rejects wallet-sized/stale-sized Max values. | No P0 missing support for current internal tester cashout. Dedicated richer cashout preview remains P1. |

## Cycle ZCD - Fresh One-Event Live Runtime Proof

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body / params | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Explicit one-event provider refresh | The Odds API `/v4/sports`, `/v4/sports/:sport/events`, `/v4/sports/:sport/events/:eventId/markets`, and `/v4/sports/:sport/events/:eventId/odds`; local `/api/mobile/events/:slug/live-detail`; `/api/markets/:marketId/quote` | Provider GET plus local GET | Provider key from ignored local runtime secret file only; local routes use internal/dev runtime access | One upcoming soccer event, region `us`, markets `h2h`, `h2h_3_way`, `spreads`, `totals`, `alternate_spreads`, and `alternate_totals`; no broad scan | Provider event identity, quota headers, normalized market/outcome IDs, reference snapshot freshness, selected market status, bid/ask, and selected outcome quote readiness | Existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `ProviderRefreshRun`, and `MarketMakerQuoteRun`. No schema change. | None. Missing provider key or a running quota-spending loop blocks refresh instead of fabricating odds. | Installed continuous provider refresh service remains P1; this is an explicit local operator command. |
| Internal tester readiness after refresh | `/api/health`; `/api/internal/live-runtime/status?phaseAuditInProgress=1`; `/api/events`; `/api/mobile/events/:slug/live-detail`; `/api/markets/:marketId/quote`; `/api/orders`; `/api/portfolio`; `/api/portfolio/history`; local `mobile:one-event-live-maker-seed` and `mobile:internal-tester-readiness-gate` | GET/POST plus local proof commands | Local fake-token mobile API/session; no provider key for no-quota readiness gate | Orders preserve selected `marketId`, `outcomeId`, side, size, and price. Maker seed targets the current selected provider-backed market. | Current event `Argentina vs. England`, selected `Total Goals 2.5` market, `Over 2.5` outcome, shifted maker bid/ask, filled fake-token buy/sell, portfolio/history state, readiness P0/P1/P2, and tester checklist identity | Existing `Order`, `Trade`, `Fill`, `Position`, `UserBalance`, plus provider/maker evidence tables. No schema change. | None. Cached readiness uses durable backend evidence; live odds freshness is reported separately. | New S23 UI proof for the fresh event remains separate from this backend/route proof. Production official-result settlement remains P1. |
| S23 mobile trading proof for fresh event | `/api/events`; `/api/mobile/events/:slug/live-detail`; `/api/markets/:marketId/quote`; `/api/orders`; `/api/portfolio`; `/api/portfolio/history`; local `mobile:one-event-cached-restore`; local counterparty seeding | GET/POST plus local DB restore/seed commands | Local fake-token mobile credential created for the proof; no provider key | Restore reads latest redacted live summary; S23 order uses selected `marketId=38664599-1911-40b4-9c57-382498da8efb`, quote-visible `outcomeId=f57b22fb-7dc8-412c-95b9-9ebf79639f03`, line `2.5`, side `BUY`; cashout uses `SELL` with owned shares only | Home event title, event detail market identity, ticket line identity, buy submit result, Portfolio position, close-position cashout markers, owned-share Max, sold activity/history | Existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `Order`, `Trade`, `Fill`, `Position`, and `UserBalance`. No schema change. | None. Cached restore is quota-free but uses the latest live provider summary shape; it does not fabricate a different event. | Per-provider-event slugs are still P2 for multi-event onboarding. |

## Cycle ZCF - Runtime Capability Matrix

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Runtime capability matrix | None. Local command `mobile:runtime-capability-matrix` reads `package.json` and runtime scripts. | Local read-only command | None; no provider key and no mobile auth | Optional `--summaryPath`; no provider/order/portfolio payload | Capability id, status, runtime mode, provider-quota behavior, start/proof/stop commands, evidence files, and current limitation | None. No Prisma schema change and no DB read/write. | None. The command reports missing scripts/source checks instead of assuming capability. | Installed unattended provider/maker/lifecycle service ownership remains P1; production official-result auto-settlement remains P1. |

## Cycle ZCG - Cached One-Event Onboarding Proof Repair

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Cached one-event onboarding runtime proof | `/api/health`; local `mobile:one-event-cached-restore`; `mobile:one-event-live-readiness`; `mobile:one-event-runtime-status`; `mobile:one-event-settlement-readiness`; `mobile:one-event-settlement`; `mobile:internal-tester-runtime` start/status/stop | GET plus local proof commands | Local developer runtime only; no provider key and no mobile auth | No provider/order payload. Runtime manager starts supervisor/result-poller and then stops them. | Active event `Argentina vs. England`, trusted-result applicability, runtime loop start/status/stop pass, backend health, Postgres health, and P0/P1/P2 gaps | Existing `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `RuntimeServiceHeartbeat`, `RuntimeServiceRun`, `MarketMakerQuoteRun`, and settlement preview state. No schema change. | Redacted replay is skipped when stale; cached live-runtime restore is used. Trusted-result ingestion/settlement is skipped when the score fixture provider event id does not match the active event. | Installed always-on provider refresh and market-maker daemons remain P1. A matching official/current result source is needed before trusted-result settlement can run for the active event. |

## Cycle ZCH - Local Runtime Persistence Capability

| Mobile/internal feature | API endpoint used | Method | Auth requirement | Request body | Response fields consumed by mobile/tooling | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Runtime capability matrix persistence rows | None. Local command `mobile:runtime-capability-matrix` reads `package.json` and local runtime scripts. | Local read-only command | None; no provider key and no mobile auth | Optional summary path only | Capability ids `local-runtime-scheduled-task` and `local-runtime-startup-launcher`, runtime mode, quota policy, start/proof/stop commands, and limitations | None | None | Production service monitoring/daemon ownership remains P1 |
| Scheduled Task install/uninstall proof | None. Local PowerShell uses Windows Scheduled Task APIs with proof task name. | Local OS command | Local Windows task-registration rights | Proof task name and summary paths | Install/uninstall result, Windows permission blocker, no persistent task left, no provider quota | None | None | Current shell cannot register scheduled task without elevated/task rights |
| User Startup launcher install/uninstall proof | None. Local PowerShell writes/removes proof `.cmd` in the user's Startup folder. | Local OS command | Current Windows user | Proof launcher name and summary paths | Install/uninstall result, launcher command includes supervisor/result-poller, no persistent launcher left, no provider quota | None | None | Production service ownership remains P1 |
