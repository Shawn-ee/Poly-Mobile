...
| Consolidated one-event live readiness | Local command `npm run mobile:one-event-live-readiness` | Local PowerShell orchestration plus existing route/service proofs | Generated local proof credentials inside child commands | Runs runtime with maker seed, lifecycle proof, backend health, Docker/Postgres, S23 reachability, cached live provider proof, and S23 visible proof checks | One redacted pass/fail readiness summary, artifact paths, runtime truth flags, P0/P1/P2 gaps | Existing proof artifacts and DB state; no schema change | Cached mode only; no provider quota spent | Continuous provider refresh daemon, continuous maker daemon, and automatic settlement remain P1. |
...
| Mobile feature | API endpoint used | Method | Auth requirement | Request body / params | Response fields consumed by mobile/proof | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Repeated local runtime and maker refresh | `npm run mobile:one-event-live-supervisor`; child command reads `/api/health` and `/api/markets/:marketId/quote` | Local command plus GET/GET | None for health/quote; optional provider proof requires `THE_ODDS_API_KEY` only in process env | `BackendPort`, `MaxIterations`, `IntervalSeconds`, optional `RunProviderProof`, optional maker seed | Backend health, Postgres health, S23 reachability, repeated runtime pass, maker bid/ask visible in quote route, selected event/market identity | `Event`, `Market`, `Outcome`, `ReferenceQuoteSnapshot`, `Order`, `User`, `UserBalance`; provider mode may update sportsbook-backed snapshots | Default cached mode spends no provider quota and uses latest stored provider snapshots. It does not fabricate new provider odds. | Supervisor is foreground local runtime only, not an installed unattended service. Automatic result settlement remains P1. |

# Cycle ONEEVENTLIFECYCLESCHEDULER - One-Event Start-Time Lifecycle Scheduler

| Mobile feature | API endpoint used | Method | Auth requirement | Request body / params | Response fields consumed by mobile/proof | Database tables/models implied | Mock fallback behavior | Missing backend support |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| One-event pause/close scheduler | `npm run mobile:one-event-lifecycle-scheduler-proof`; proof checks `/api/health` and `POST /api/orders` rejection behavior | Local command plus GET/POST | Generated local mobile API credential for order rejection proof | `eventSlug=odds-api-single-soccer-test`, temporary event start times, suspend-before-start threshold | Scheduler action `none/pause/close`, changed market count, canceled order count, paused/closed order rejection, restore checks | `Event.startTime`, `Market.status`, `Market.closeTime`, `Order`, `User`, `UserBalance`, `ApiCredential`; close path uses existing open-order cancellation cleanup | No provider quota and no fake provider data. The proof temporarily mutates event/market lifecycle state, then restores and reseeds local maker quotes. | Scheduler is local callable logic, not an installed always-on service. Official-result settlement remains manual/future. |
