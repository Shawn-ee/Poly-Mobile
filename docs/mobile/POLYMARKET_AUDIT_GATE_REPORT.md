...
| Event market catalog contract | Cycle KH | Pass for backend/data-contract scope | 0 for focused Event Detail/Game Lines market catalog route/service contract | P1 wire dirty Event Detail UI files to `loadEventMarketCatalog()` after screen churn is reconciled; P1 optional Android line-chip route-refresh proof if visual proof becomes required again | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Event market catalog proof: `docs/mobile/harness/cycle-KH-event-market-catalog-contract/cycle-KH-event-market-catalog-contract.json`; tests: `mobile/src/__tests__/api.test.ts`, `mobile/src/__tests__/eventMarketCatalogService.test.ts`, `mobile/src/__tests__/marketLineOptionsService.test.ts`, `src/__tests__/public.event-markets.no-leak.test.ts`; audit: `mobile/docs/audits/cycle-KH-event-market-catalog-contract.md` | Mobile `loadEventMarketCatalog()` prefers `/api/events/:slug/markets`, preserves backend Game Lines metadata, filters private/unlisted rows server-side, and keeps successful empty route results authoritative. |
| Event Detail hydration contract | Cycle KG | Pass for backend/data-contract scope | 0 for focused Event Detail hydration route/client contract | P1 wire dirty Event Detail UI files to compact live-detail hydration after screen churn is reconciled; P1 production real-provider replay remains in provider lanes | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Event Detail hydration proof: `docs/mobile/harness/cycle-KG-event-detail-hydration-contract/cycle-KG-event-detail-hydration-contract.json`; tests: `mobile/src/__tests__/api.test.ts`, `mobile/src/__tests__/worldCupAdapter.test.ts`, `src/__tests__/mobile-live-event-detail.test.ts`, `src/__tests__/mobile-event-market-rules-contract.test.ts`; audit: `mobile/docs/audits/cycle-KG-event-detail-hydration-contract.md` | Mobile `PolyApi.getEvent()` prefers `/api/mobile/events/:slug/live-detail` and keeps legacy fallback; live-detail route proof carries backend event rules, regulation draw outcome, and spread compact market rows. |
| Ticket quote route contract | Cycle KF | Pass for backend/data-contract scope | 0 for focused ticket quote route/service contract | P1 wire dirty visible Trade Ticket/Event Detail quote refresh behavior after screen churn is reconciled; P1 production provider quote breadth remains in provider lanes | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Ticket quote proof: `docs/mobile/harness/cycle-KF-ticket-quote-route-contract/cycle-KF-ticket-quote-route-contract.json`; tests: `mobile/src/__tests__/quoteService.test.ts`, `mobile/src/__tests__/api.test.ts`, `src/__tests__/market.quote.route.test.ts`, `src/__tests__/orderbook-pricing.quote-size.test.ts`; audit: `mobile/docs/audits/cycle-KF-ticket-quote-route-contract.md` | Mobile `loadTicketQuotes()` now has route proof through `/api/markets/:id/quote?outcomeId=...`, preserving best bid/ask, top-of-book sizes, midpoint probability, and last trade price for ticket pricing. |
| Portfolio sync route contract | Cycle KE | Pass for backend/data-contract scope | 0 for focused Portfolio sync route/service contract | P1 wire dirty Portfolio UI files to `loadServerPortfolioState()` after screen churn is reconciled; P1 optional Android proof if visual proof becomes required again | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Portfolio sync proof: `docs/mobile/harness/cycle-KE-portfolio-sync-route-contract/cycle-KE-portfolio-sync-route-contract.json`; tests: `mobile/src/__tests__/portfolioSyncService.test.ts`, `mobile/src/__tests__/portfolioSnapshotService.test.ts`, `mobile/src/__tests__/portfolioHistoryService.test.ts`, `src/__tests__/portfolio.open-orders.route.test.ts`, `src/__tests__/portfolio.history.route.test.ts`; audit: `mobile/docs/audits/cycle-KE-portfolio-sync-route-contract.md` | Mobile `loadServerPortfolioState()` now has route proof across `/api/portfolio` and `/api/portfolio/history`, preserving backend selection metadata through positions, open orders, canceled activity, and recent trade activity. |
| Home event filter contract | Cycle KD | Pass for backend/data-contract scope | 0 for focused Home status-filter route/service contract | P1 wire dirty Home/Live UI files to `loadHomeEventFeedPage()` in server mode; P1 calendar-accurate today filtering if product needs a date-window tab | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Home filter proof: `docs/mobile/harness/cycle-KD-home-event-filter-contract/cycle-KD-home-event-filter-contract.json`; tests: `mobile/src/__tests__/homeEventFeedService.test.ts`, `mobile/src/__tests__/api.test.ts`, `src/__tests__/public.events.no-leak.test.ts`; audit: `mobile/docs/audits/cycle-KD-home-event-filter-contract.md` | Mobile `loadHomeEventFeedPage()` now prefers `/api/events?status=...&includeMobileMarkets=1&limit=...&cursor=...` route pages for Home filter feeds, with fallback isolated to route-unavailable mode. |
| Profile summary contract | Cycle KC | Pass for backend/data-contract scope | 0 for focused Account summary route/service contract | P1 wire dirty Account UI files to `loadProfileSummary()` in server mode; P1 full account/security/session/funding settings remain outside focused MVP scope | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Profile summary proof: `docs/mobile/harness/cycle-KC-profile-summary-contract/cycle-KC-profile-summary-contract.json`; tests: `mobile/src/__tests__/profileSummaryService.test.ts`, `mobile/src/__tests__/api.test.ts`, `src/__tests__/profile.summary.route.test.ts`; audit: `mobile/docs/audits/cycle-KC-profile-summary-contract.md` | `/api/profile/summary` now returns canonical profile identity, preferences, wallet/account totals, open position/order counts, exposure, and server trading mode for the visible Account shell. |
| Search event service contract | Cycle KB | Pass for backend/data-contract scope | 0 for focused Search service route-loading contract | P1 wire dirty Search UI files to the service in server mode; P1 ranked/faceted discovery only if World Cup MVP Search scope expands | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Search service proof: `docs/mobile/harness/cycle-KB-search-event-service-contract/cycle-KB-search-event-service-contract.json`; tests: `mobile/src/__tests__/searchEventService.test.ts`, `mobile/src/__tests__/api.test.ts`, `src/__tests__/public.events.no-leak.test.ts`; audit: `mobile/docs/audits/cycle-KB-search-event-service-contract.md` | Mobile `loadSearchEventPage()` now prefers `/api/events?search=...&limit=...&cursor=...` route data with compact markets and cursor pagination; local filtering is isolated to route-unavailable fallback. |
| Trade Ticket submit route contract | Cycle KA | Pass for backend/data-contract scope | 0 for focused submit route/service contract | P1 Android proof that visible Trade Ticket submit gesture uses HTTP route after dirty UI churn is reconciled; P1 broader provider-family submit breadth if needed | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Submit route proof: `docs/mobile/harness/cycle-KA-trade-ticket-submit-route-contract/cycle-KA-trade-ticket-submit-route-contract.json`; tests: `mobile/src/__tests__/orderService.test.ts`, `mobile/src/__tests__/api.test.ts`, `src/__tests__/orders.internal-trading-gate.route.test.ts`; audit: `mobile/docs/audits/cycle-KA-trade-ticket-submit-route-contract.md` | Mobile `submitTicketOrder()` reaches real `POST /api/orders`; internal trading gate and provider accepting quote check pass; `/api/portfolio` exposes the new open order with selected totals identity. |
...
| Mobile API route contract backfill | Cycle JV | Pass for backend/data-contract consolidation scope | 0 for focused mobile client/type route contracts | P1 Search UI backend pagination; P1 Portfolio value-history UI route loading after unrelated UI churn is reconciled | Product decision on 2026-07-06: manual UI review is no longer required for every backend-wiring cycle | Route/client proof: `docs/mobile/harness/cycle-JV-mobile-api-route-contract-backfill/cycle-JV-mobile-api-route-contract-backfill.json`; tests: `mobile/src/__tests__/api.test.ts`, `src/__tests__/public.events.no-leak.test.ts`, `src/__tests__/portfolio.value-history.route.test.ts`; audit: `mobile/docs/audits/cycle-JV-mobile-api-route-contract-backfill.md` | Mobile API/types now carry committed support for event pagination, backend market-rule fields, portfolio value-history route shape, and Portfolio selection identity fields. |
...
Scope: Backend/data-contract gate for visible account/cash balance loading through canonical `/api/account/balance`. This does not claim visible Account or Portfolio UI wiring.
...
- Remaining P1: wire dirty Account/Portfolio UI files to `loadAccountBalance()` where standalone balance refresh is needed; keep legacy `/api/wallet/balance` compatibility-only until canonical adoption is complete.
...
- Remaining P1: wire dirty Event Detail UI files to `loadEventMarketCatalog()` after unrelated screen churn is reconciled; add Android line-chip route-refresh proof only if visual proof becomes required again.
...
- Remaining P1: wire dirty Event Detail UI files to compact live-detail hydration after unrelated screen churn is reconciled; keep production real-provider replay under provider mapping/provider refresh lanes.
...
Scope: Backend/data-contract gate for Home event feed status filters through `/api/events`. This does not claim visible Home/Live UI server-mode wiring.
...
- Remaining P1: wire dirty Home/Live UI files to `loadHomeEventFeedPage()` in server mode; add calendar-accurate `today` route filtering only if product needs date-window tabs instead of status tabs.

## Cycle KC

Gate status: Pass

Scope: Backend/data-contract gate for visible Account/profile summary route and mobile mapper. This does not claim Account UI server-mode wiring or funding/settings expansion.
...
- Remaining P1: wire dirty Account UI files to `loadProfileSummary()` in server mode; broader account/security/session/funding settings remain outside this focused MVP summary route.
...
- Remaining P1: wire dirty Search UI files to `loadSearchEventPage()` in server mode after unrelated UI churn is reconciled; ranked/faceted discovery only if World Cup MVP Search scope expands.
...
- Remaining P1: Android proof that the visible Trade Ticket submit gesture uses this route in server mode after dirty UI churn is reconciled, and broader provider-family submit breadth if future gates require it.
...
Scope: Backend/data-contract gate for the Portfolio value-history mobile service loader. This does not claim Portfolio UI wiring or visual redesign.
...
- Remaining P1: wire dirty Portfolio UI to `loadPortfolioValueHistory()` and prove the visible chart consumes `source=portfolio-value-history-route` in server mode.
...
- Remaining P1: wire dirty Event Detail/Game Lines UI to the service and prove visible line chips follow backend availability.
...
- Remaining P1: UI-level Portfolio proof after dirty screen churn is reconciled and broader real-provider lifecycle repetition.
...
- Remaining P1: Search UI backend pagination and Portfolio value-history UI route loading after unrelated UI churn is reconciled.
