...
| --- | --- | --- | --- | --- |
| Account menu availability wiring | `mobile/docs/audits/cycle-KY-account-menu-availability-wiring.md` | Pass for focused backend/data-contract scope. Visible Account More-menu rows now consume `/api/profile/summary` `menuItems[]` availability metadata and render outside-MVP rows as non-actionable unavailable rows instead of dead buttons. | P1 implement real backend routes for menu items only if MVP scope expands | Cycle KY pass |
| Route wiring tracker consolidation | `mobile/docs/audits/cycle-KX-route-wiring-tracker-consolidation.md` | Pass for documentation/audit scope. Stale tracker notes that claimed already-closed Search, Home filter, Portfolio, Trade Ticket, Account, and Event Detail UI wiring remained open are reconciled to their closing KJ-KW cycles. | P1 run another tracker sweep after the next backend/UI wiring batch | Cycle KX pass |
| Profile preferences UI sync wiring | `mobile/docs/audits/cycle-KW-profile-preferences-ui-sync-wiring.md` | Pass for focused backend/data-contract scope. Visible server-mode preference state loads and saves locale, ticket defaults, slippage, and saved market ids through canonical `/api/profile/preferences`, with Account displaying route-backed values and sync status. | P1 broader account/security/session/funding settings only if visible MVP scope expands; optional Android proof if visual proof becomes required again | Cycle KW pass |
| Home filter UI route wiring | `mobile/docs/audits/cycle-KV-home-filter-ui-route-wiring.md` | Pass for focused backend/data-contract scope. Visible Home `All/Live/Today` chips drive `loadHomeEventFeedPage({ filter: homeFilter })` in server mode, and successful backend pages are no longer locally filtered from only the already-loaded Home page. | P1 optional Android proof if visual proof becomes required again; calendar-accurate `today` date-window semantics only if product later wants date-window filtering instead of status filtering | Cycle KV pass |
| Portfolio value-history UI wiring | `mobile/docs/audits/cycle-KU-portfolio-value-history-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Portfolio chart server mode consumes `loadPortfolioValueHistory()` and exposes route source/status metadata from `displayedValueHistory`. | P1 optional Android proof if visual proof becomes required again; persisted account-level value snapshots remain future hardening | Cycle KU pass |
| Account balance UI wiring | `mobile/docs/audits/cycle-KT-account-balance-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Portfolio cash balance and bottom-tab portfolio value refresh from canonical `/api/account/balance` through `loadAccountBalance()` in server mode. | P1 legacy `/api/wallet/balance` cleanup after non-mobile web wallet compatibility review | Cycle KT pass |
| Event Detail line options UI wiring | `mobile/docs/audits/cycle-KS-event-detail-line-options-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Event Detail Spread/Totals line and period chips derive from backend `event.markets` through `marketLineOptionsService`; static frontend line rails are removed from the committed component path. | P1 optional Android proof if visual proof becomes required again; production real-provider breadth remains under provider lanes | Cycle KS pass |
| Portfolio cancel UI wiring | `mobile/docs/audits/cycle-KR-portfolio-cancel-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Portfolio open-order cancel calls `cancelOpenOrderOnServer()` and canonical `DELETE /api/orders/:id` in server mode, then refreshes Portfolio from backend state. | P1 broader provider-family cancel breadth if future gates require it; optional Android proof if visual proof becomes required again | Cycle KR pass |
| Trade Ticket submit UI wiring | `mobile/docs/audits/cycle-KQ-trade-ticket-submit-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Trade Ticket submit calls `placeOrder()`, which reaches `submitTicketOrder()` and canonical `POST /api/orders` in server mode, then refreshes Portfolio from backend state. | P1 broader provider-family submit breadth if future gates require it; optional Android proof if visual proof becomes required again | Cycle KQ pass |
| Portfolio sync UI wiring | `mobile/docs/audits/cycle-KP-portfolio-sync-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Portfolio server mode consumes `/api/portfolio` and `/api/portfolio/history` through `loadServerPortfolioState()` and passes route-backed balance, positions, open orders, activities, and sync status to `Portfolio`. | P1 optional Android proof if visual proof becomes required again; broader provider lifecycle breadth remains under provider lanes | Cycle KP pass |
| Trade Ticket quote UI wiring | `mobile/docs/audits/cycle-KO-trade-ticket-quote-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Trade Ticket server mode consumes `/api/markets/:id/quote?outcomeId=...` through `loadTicketQuotes()` and scopes quote updates to the still-open ticket market/outcome. | P1 optional Android proof if visual proof becomes required again; production provider quote breadth remains under provider lanes | Cycle KO pass |
| Event Detail catalog UI wiring | `mobile/docs/audits/cycle-KN-event-detail-catalog-ui-wiring.md` | Pass for focused backend/data-contract scope. Visible Event Detail server mode consumes `/api/events/:slug/markets` through `loadEventMarketCatalog()` and treats backend catalog rows as authoritative for Game Lines. | P1 optional Android proof if visual proof becomes required again; production real-provider breadth remains under provider lanes | Cycle KN pass |
| Event Detail UI hydration wiring | `mobile/docs/audits/cycle-KM-event-detail-ui-hydration-wiring.md` | Pass for focused backend/data-contract scope. Visible Event Detail server mode calls `PolyApi.getEvent()`, prefers compact live-detail hydration, and preserves backend event rules/markets before updating the selected event. | P1 explicit visible Game Lines catalog refresh from `/api/events/:slug/markets`; optional Android proof if visual proof becomes required again | Cycle KM pass |
| Account UI summary wiring | `mobile/docs/audits/cycle-KL-account-ui-summary-wiring.md` | Pass for focused backend/data-contract scope. Visible Account screen server mode consumes `/api/profile/summary` for cash/account summary props and clears stale route summary state on failure. | P1 broader account/security/session/funding settings only if visible MVP scope expands | Cycle KL pass |
| Live UI route wiring | `mobile/docs/audits/cycle-KK-live-ui-route-wiring.md` | Pass for focused backend/data-contract scope. Visible Live tab server mode consumes backend `/api/events?status=live&limit=10` pages and refreshes from that route instead of filtering only the loaded Home page. | P1 optional Android proof if visual proof becomes required again; rich live sports-stat feeds remain outside this route-wiring cycle | Cycle KK pass |
...
| Account balance route contract | `mobile/docs/audits/cycle-KI-account-balance-route-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadAccountBalance()` reads canonical `/api/account/balance`, maps available/locked/total USDC into numeric visible cash values, and keeps local balance fallback route-unavailable only. Cycle KT wires visible Portfolio/bottom-tab balance state to the service. | P1 legacy wallet-balance cleanup after canonical route adoption | Cycle KI pass; Cycle KT UI wiring pass |
| Event market catalog contract | `mobile/docs/audits/cycle-KH-event-market-catalog-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadEventMarketCatalog()` prefers `/api/events/:slug/markets`, normalizes public listed route markets into Game Lines market metadata, and keeps successful empty route results empty. | Cycle KN wires visible Event Detail/Game Lines catalog refresh; remaining P1 is optional Android line-chip route-refresh proof if visual proof becomes required again | Cycle KH pass; Cycle KN UI wiring pass |
| Event Detail hydration contract | `mobile/docs/audits/cycle-KG-event-detail-hydration-contract.md` | Pass for focused backend/data-contract scope. Mobile `PolyApi.getEvent()` prefers `/api/mobile/events/:slug/live-detail`, keeps legacy `/api/events/:slug` as failure fallback only, and the compact route carries backend-owned market rules plus compact regulation draw and spread rows. | Cycle KM wires visible Event Detail hydration; remaining P1 production real-provider replay remains under provider lanes | Cycle KG pass; Cycle KM UI wiring pass |
| Ticket quote route contract | `mobile/docs/audits/cycle-KF-ticket-quote-route-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadTicketQuotes()` reads `/api/markets/:id/quote`, maps best bid/ask, top-of-book sizes, midpoint probability, and last trade price into ticket quote fields. Cycle KO proves visible Trade Ticket/Event Detail quote refresh wiring. | P1 production provider quote breadth remains under provider lanes | Cycle KF pass |
| Portfolio sync route contract | `mobile/docs/audits/cycle-KE-portfolio-sync-route-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadServerPortfolioState()` reads backend `/api/portfolio` plus `/api/portfolio/history`, maps positions/open orders/activities, and preserves backend selection metadata. Cycle KP proves visible Portfolio UI wiring. | P1 optional Android proof if visual proof becomes required again; broader provider lifecycle breadth remains under provider lanes | Cycle KE pass |
| Home event filter contract | `mobile/docs/audits/cycle-KD-home-event-filter-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadHomeEventFeedPage()` prefers backend `/api/events?status=...&limit=...&cursor=...` pages, preserves compact markets, and isolates local filtering to route-unavailable fallback. | Cycle KV wires visible Home chips to the service; remaining P1 is calendar-accurate today filtering only if product keeps a date-window tab | Cycle KD pass; Cycle KV UI wiring pass |
| Profile summary contract | `mobile/docs/audits/cycle-KC-profile-summary-contract.md` | Pass for focused backend/data-contract scope. `/api/profile/summary` returns canonical profile identity, preferences, wallet/account totals, open position/order counts, open order value, total exposure, and mobile `loadProfileSummary()` maps it to Account values. | Cycle KL wires Account summary UI; broader account/security/session/funding settings remain outside focused MVP scope | Cycle KC pass; Cycle KL UI wiring pass |
| Search event service contract | `mobile/docs/audits/cycle-KB-search-event-service-contract.md` | Pass for focused backend/data-contract scope. Mobile `loadSearchEventPage()` prefers backend `/api/events?search=...&limit=...&cursor=...` pages, preserves compact markets, and isolates local filtering to route-unavailable fallback. | Cycle KJ wires the visible Search tab; remaining P1 ranked/faceted discovery only if World Cup MVP Search scope expands | Cycle KB pass; Cycle KJ UI wiring pass |
| Trade Ticket submit route contract | `mobile/docs/audits/cycle-KA-trade-ticket-submit-route-contract.md` | Pass for focused backend/data-contract scope. Mobile `submitTicketOrder()` reaches real `POST /api/orders`, passes internal trading/provider availability gates, and Portfolio hydrates the created open order with selected totals identity. Cycle KQ proves visible Trade Ticket submit UI wiring. | P1 broader provider-family submit breadth if needed; optional Android proof if visual proof becomes required again | Cycle KA pass |
| Open order cancel route contract | `mobile/docs/audits/cycle-JZ-open-order-cancel-route-contract.md` | Pass for focused backend/data-contract scope. Server-mode mobile cancel calls `DELETE /api/orders/:id`, Portfolio removes the open order after cancel, and history/activity preserve selected market identity. Cycle KR proves visible Portfolio cancel UI wiring. | P1 broader provider-family cancel breadth if needed; optional Android proof if visual proof becomes required again | Cycle JZ pass |
| Portfolio value-history service contract | `mobile/docs/audits/cycle-JY-portfolio-value-history-service-contract.md` | Pass for focused backend/data-contract scope. Mobile service prefers `/api/portfolio/value-history?range=...` route data when an API client is available and keeps deterministic history as fallback only. Cycle KU wires the visible Portfolio chart to the service. | P1 optional Android proof if visual proof becomes required again; persisted account-level value snapshots remain future hardening | Cycle JY pass; Cycle KU UI wiring pass |
| Line options contract | `mobile/docs/audits/cycle-JX-line-options-contract.md` | Pass for focused backend/data-contract scope. Mobile derives line/period options from backend markets only and rejects wrong-period matches. Cycle KS wires the visible Event Detail/Game Lines chips to this service. | P1 optional Android proof if visual proof becomes required again; production real-provider breadth remains under provider lanes | Cycle JX pass; Cycle KS UI wiring pass |
| Portfolio activity mapper contract | `mobile/docs/audits/cycle-JW-portfolio-activity-mapper-contract.md` | Pass for focused backend/data-contract scope. Portfolio service mappers preserve backend `to_advance` selection identity and aggregate multi-fill recent trades into one retail activity row. | Cycle KP proves visible Portfolio sync UI wiring; remaining P1 broader real-provider lifecycle repetition | Cycle JW pass; Cycle KP UI wiring pass |
| Mobile API route contract backfill | `mobile/docs/audits/cycle-JV-mobile-api-route-contract-backfill.md` | Pass for focused backend/data-contract consolidation scope. Mobile API/types now commit route contracts for event pagination, backend market-rule fields, portfolio value-history, and Portfolio selection identity fields. | Search UI backend pagination is closed by Cycle KJ, Portfolio value-history UI route loading by Cycle KU, and Home filter UI route loading by Cycle KV | Cycle JV pass; KJ/KU/KV UI wiring pass |
| Profile preferences route contract | `mobile/docs/audits/cycle-JU-profile-preferences-route-contract.md` | Pass for focused backend/data-contract scope. Mobile/server preference payloads preserve locale, ticket default amount, ticket side, slippage, and saved event ids through canonical `/api/profile/preferences` read/write shape. | Cycle KW wires visible preference state to the route; broader account/settings shell remains future scope only if MVP expands | Cycle JU pass; Cycle KW UI wiring pass |
| Search event route contract | `mobile/docs/audits/cycle-JT-search-event-route-contract.md` | Pass for focused backend/data-contract scope. `/api/events?search=` matches event/team/market/outcome text, returns compact mobile markets, and preserves cursor pagination. | Cycle KJ wires the visible Search tab to backend pages; remaining P1 ranked/faceted production discovery if needed | Cycle JT pass; Cycle KJ UI wiring pass |
...
| FD-DISC-P0-03 | P0 | Server market-data mode must hydrate the opened detail through `/api/mobile/events/:slug/live-detail`. | Android XML showing route-backed probability/outcome and provider markers from the live-detail payload. |
| FD-DISC-P0-04 | P0 | The opened detail must expose the local MVP retail surface: probability/outcome display, Game Lines, and tradeable outcomes, without the market-page chart. | Screenshot/XML with `Game Lines`, provider-backed outcome labels, provider source marker, and absence of `event-detail-price-chart`. |
...
| FE-TICKET-P0-02 | P0 | Tapping the Home card must open the same route-backed Event Detail. | Android screenshot/XML after tap showing same event title, probability/outcome display, Game Lines, and absence of `event-detail-price-chart`. |
