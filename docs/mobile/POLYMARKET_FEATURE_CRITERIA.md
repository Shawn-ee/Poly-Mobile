...
| --- | --- | --- | --- | --- |
| Live event visible provider lifecycle/status parity EH | `docs/mobile/audits/cycle-eh-c-provider-status-gate.md` | Fail until integrated Android-visible proof. P0 requires ready, stale/refresh-due, refreshing/loading, and unavailable/not-ready states visibly tied to the same selected market identity through live page, chart, Book/orderbook, and ticket. Backend-only lifecycle proof, generic fallback rows, fixture/mock-ready data, default moneyline reconstruction, first-row fallback, and event-only labels fail. | P1/P2 remaining: fresh official S23 recapture, broader real provider-backed line-family status matrix, actual stale -> refreshing/loading -> ready transition proof, and status visual polish | EH-C fail-until-integrated-proof; PM-GAP-084 remains open |
...
The focused EH-C gate in `docs/mobile/audits/cycle-eh-c-provider-status-gate.md` tracks the remaining PM-GAP-084 blocker after EG: Android-visible provider lifecycle/status parity. Current status is fail until integrated Android proof exists.
