...
| --- | --- | --- | --- | --- |
| Orderbook family/depth selector DS-C | `docs/mobile/audits/cycle-ds-c-orderbook-audit-gate.md` | Partial integrated proof exists for dedicated Book surface, event identity, visible Yes/No tabs, grouped selector labels, Price/Shares/Value ladder, spread separator, fallback/unavailable states, and ticket action; PM-GAP-075 remains open | P1 `Decimalize book`, broader selector coverage, row-to-ticket carry-through, and P2 visual polish remain open; remaining P0 proof needed for tab switch, selector carry-through, settings, and provider-backed ready depth | Partial; not pass |
...
The focused DS-C gate in `docs/mobile/audits/cycle-ds-c-orderbook-audit-gate.md` converts DQ-C Book/orderbook evidence into pass/fail criteria for PM-GAP-075. Current status is partial after integrated Android proof in `docs/mobile/harness/cycle-DS-integrated-orderbook-ui-proof.json`; PM-GAP-075 is not passed yet.
