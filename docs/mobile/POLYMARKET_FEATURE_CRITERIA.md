...
| --- | --- | --- | --- | --- |
| Book-selected order to Portfolio/history ED lifecycle | `docs/mobile/audits/cycle-ed-c-book-order-portfolio-gate.md` | Open/fail until integrated proof. P0 requires live game page Book-selected market/line/outcome -> ticket -> fake-token order submit -> Portfolio open order/open position -> activity/history, with Android-visible proof and backend order/portfolio/history route proof for the same selected identity. Backend-only or screenshot-only proof cannot pass | P1/P2 remaining: real provider-backed line-family lifecycle breadth, open/cancel/fill status breadth, production confirmation recapture, immutable selection snapshots, and Portfolio/history visual polish | ED-C gate prepared; PM-GAP-081 open |
