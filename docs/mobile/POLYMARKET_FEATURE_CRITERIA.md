...
| --- | --- | --- | --- | --- |
| Live event route-backed provider lifecycle/status parity EI | `docs/mobile/audits/cycle-ei-c-route-backed-status-gate.md` | Fail-until-integrated-proof. P0 remains open until tablet-visible provider status is sourced from the live backend route, with backend health/reachability from tablet launch context, route-backed markers in tablet XML/proof, same selected market identity through live page/chart/Book/ticket, and no deterministic fixture/mock-ready/default fallback. | P1/P2 remaining: fresh official S23 recapture, broader real provider-backed line-family status matrix, actual route-backed stale -> refreshing/loading -> ready transition proof, and status visual polish | EI-C docs gate; PM-GAP-084 remains open |
...
The focused EI-C gate in `docs/mobile/audits/cycle-ei-c-route-backed-status-gate.md` tracks the remaining PM-GAP-084 blocker after EH: tablet-visible provider lifecycle/status must be sourced from the live backend route. Current status is fail-until-integrated-proof.
