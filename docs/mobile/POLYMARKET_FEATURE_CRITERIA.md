...
| --- | --- | --- | --- | --- |
| Book-origin order status breadth and selection snapshots EE | `docs/mobile/audits/cycle-ee-c-book-order-status-gate.md` | Fail until integrated proof. P0 requires Android-visible proof plus backend route/data proof for the same Book-origin selected identity across open order, cancel/canceled status, filled position, recent activity/history, order-time/fill-time selection snapshots, and no fallback | P1/P2 remaining: real provider-backed line-family status matrix, official production confirmation/cancel/fill recapture, durability checks after metadata changes, and Portfolio/history visual status polish | EE-C gate opened PM-GAP-082; not parity-pass |
