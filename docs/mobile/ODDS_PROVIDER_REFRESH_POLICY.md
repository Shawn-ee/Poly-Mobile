...
- `npm run mobile:one-event-live-supervisor` repeats the local one-event runtime check and maker seed without spending provider quota unless `-RunProviderProof` is passed.
