...
- Cached onboarding must not use a trusted-result score fixture unless the fixture provider event id matches the active backend provider event id. If it does not match, result ingestion and trusted-result settlement dry-run are skipped explicitly.
...
- `npm run mobile:one-event-live-runtime:provider` is the lower-level explicit live provider proof command and requires `THE_ODDS_API_KEY` in the process environment. It now runs the no-quota live-odds refresh preflight before spending quota and refuses to continue if another quota-spending loop is already reported running.
...
- `npm run mobile:one-event-live-runtime:provider-secret` runs the same explicit live provider proof while loading the key from the process environment or from `.runtime/secrets/the-odds-api-key.txt`. Before spending quota, the wrapper runs the no-quota live-odds refresh preflight into `.runtime` and refuses to continue if another quota-spending loop is already reported running, then delegates to the lower-level provider command with the duplicate preflight skipped. The key is never passed on the command line, never printed, and the `.runtime` path is git-ignored.
...
- Local persistence managers default to no provider quota. Scheduled Task and Startup launcher plans may only spend provider quota when live provider flags are explicitly provided and `THE_ODDS_API_KEY` is available in the process/user environment; keys are not read from committed files or printed.
