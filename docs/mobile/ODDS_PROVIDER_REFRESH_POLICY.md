...
- `npm run mobile:one-event-live-runtime:provider-secret` runs the same explicit live provider proof while loading the key from the process environment or from `.runtime/secrets/the-odds-api-key.txt`. Before spending quota, the wrapper runs the no-quota live-odds refresh preflight into `.runtime` and refuses to continue if another quota-spending loop is already reported running. The key is never passed on the command line, never printed, and the `.runtime` path is git-ignored.
