...
| Current warm runtime proof | Added for internal tester launch | `npm run mobile:current-runtime-state-proof` restores the Spain vs. France cached event, starts the local supervisor and result poller directly, proves both process loops are running without provider quota, and then stops both loops. Backend status-route warmth is tracked as P1 because the route still reads persisted heartbeat state rather than live process-state files. |
...
- Warm runtime limitation: `/api/internal/live-runtime/status` still reports persisted heartbeat/status-route warmth as P1 rather than live process-state truth. The direct process proof is the current source of truth for local loop liveness.
