...
- Result: pass.
- Provider event: Argentina vs. England, `soccer_fifa_world_cup`, starts `2026-07-15T19:00:00Z`.
...
- Active event settlement clone proof: the active Argentina vs. England Total Goals 2.5 market shape is cloned into a disposable event, approved trusted-result auto-execution resolves the clone after close, and the active tester market remains `LIVE`, unresolved, and untouched.
...
- Current warm runtime proof: `npm run mobile:current-runtime-state-proof` restores the current cached live-runtime event, starts both local background loops directly, proves supervisor and result poller process state is running, confirms provider quota is not used, and stops both loops after proof.
...
- Latest explicit live provider refresh proof ran `npm run mobile:one-event-live-runtime:provider-secret`, spent 13 credits under a 16-credit cap, left 229 provider requests remaining, and refreshed the selected event back to ready. The no-quota maker/status summaries now report clean mobile label `Over 2.5` and retain raw provider label `Over +2.5` as reference metadata.
...
- S23 proof: `docs/mobile/harness/cycle-ZCD-s23-live-event-proof/cycle-ZCD-S23-LIVE-odds-api-s23-visible-flow.json`.
- Latest selected-outcome refresh: after Cycle ZAN S23 cashout proof, `npm run mobile:one-event-runtime-status` detected that the prior maker summary referenced a stale outcome id. `npm run mobile:one-event-live-maker-seed` refreshed the selected market to current `Over 2.5` outcome id `cf189c8a-cadc-4742-9f1f-e64a4911f228`, left visible quote-route bid/ask `0.58/0.60`, and the ordered live-runtime audit gate passed with zero P0 gaps.
