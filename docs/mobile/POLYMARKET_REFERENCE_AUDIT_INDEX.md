...
| --- | --- | --- | --- | --- | --- | --- |
| Orderbook family/depth selector DS-C | `docs/mobile/audits/cycle-ds-c-orderbook-audit-gate.md` | Criteria prepared from fresh DQ-C S23 Book reference; pending integrated Agent B Holiwyn proof | Samsung S23 / official Polymarket Android app from DQ-C | Not yet provided for DS-C | Fail until proof | PM-GAP-075 requires Book action, Yes/No tabs, grouped Moneyline/Spreads selector, family/period/line identity carry-through, Price/Shares/Value ladder, red/green bid/ask sides, spread separator, `Decimalize book` or equivalent setting, and ready/non-ready state proof under `cycle-DS-C-*`. |
