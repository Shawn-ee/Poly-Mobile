...
| One-event result review trail | `scripts/report_odds_api_one_event_result_review_trail.ts` reads canonical `provider.result.ingested` and `settlement.trusted_result.preflight` events for the active event and selected market. | Proven read-only without provider quota. It gives an operator one local report that connects provider result evidence, settlement preflight evidence, selected market/outcome, current market status, and the next execution decision. It is not a dedicated official-result table or review UI. |
