...
| Durable runtime heartbeat mirror | `GET /api/internal/live-runtime/status` upserts `RuntimeServiceHeartbeat` rows for the one-event supervisor and dedicated result poller after checking local process-state files and OS pid state. | Proven without provider quota. The current rows record both local services as stopped, no installed OS service, and no quota-spending loop. This gives durable local status evidence for internal tooling, but it is still a mirror of local process checks rather than production service ownership. |
...
- Local live-runtime status also persists a durable `RuntimeServiceHeartbeat` mirror for the supervisor and result poller so internal status survives beyond ephemeral `.runtime` files. The rows explicitly record that no quota-spending loop and no installed OS service are currently running.
