...
| Local runtime launch profile | `scripts/report_holiwyn_local_runtime_launch_profile.ts` reads the runtime status, internal tester manager, supervisor/result-poller, Startup launcher, and scheduled-task proof artifacts. | Proven read-only without provider quota. It recommends the user Startup fallback in the current Windows context, records the scheduled-task permission blocker, lists manual foreground commands, and keeps live-provider mode explicit/opt-in. |
| Live-runtime completion audit | `scripts/report_holiwyn_live_runtime_completion_audit.ts` reads runtime status, live provider proof, readiness, launch profile, internal tester watchdog, stale guard, lifecycle, active-settlement readiness, and S23 proof artifacts. | Proven read-only without provider quota. It directly answers the phase completion questions for maker continuity, live/replay odds mode, refresh cadence, quota protection, stale handling, lifecycle, local watchdog health, and mobile trade proof while keeping unattended service and production official-result auto-settlement as P1. |
...
- Result: pass.
- The proof is bounded and local-only. It proves the minimum live path for one upcoming provider event and now has a foreground supervisor that repeats hygiene, cached runtime checks, shifted maker seeding, safe lifecycle scheduling, provider-shaped result ingestion, trusted-result settlement dry-runs, and approval-file settlement wait checks while it runs. It also has canonical result-ingestion, settlement-preflight, and settlement-approval audit evidence. A dedicated result poller repeats result ingestion plus settlement scheduling with no provider quota by default, can be managed as a hidden local background process, and is now included in both the current-user Startup launcher fallback and the Windows scheduled-task plan profile with approved-settlement wait mode. Settlement execution is proven separately on fresh disposable local markets so the active tester event is not mutated. The current Windows process still cannot register the scheduled task, so this remains a local tester runtime rather than an unattended production daemon.
- Local live-runtime status also persists durable `RuntimeServiceHeartbeat` rows for the supervisor and result poller so internal status survives beyond ephemeral `.runtime` files. The local loops now emit worker-owned heartbeat metadata, and the status route preserves it while recording that no quota-spending loop and no installed OS service are currently running.
