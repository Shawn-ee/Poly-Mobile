...
| Dedicated local result poller | `scripts/run_holiwyn_one_event_result_poller.ps1` repeatedly runs provider-shaped score ingestion and trusted-result settlement scheduling, with replay/no-quota default mode and explicit quota-capped live score polling via `-RunLiveResultIngestion`. | Proven locally for two cycles without provider quota. It produces its own summary and heartbeat, regenerates trusted result JSON each cycle, dry-runs settlement scheduling each cycle, and does not execute active tester settlement. It is a local runner, not an installed unattended official-result service. |
