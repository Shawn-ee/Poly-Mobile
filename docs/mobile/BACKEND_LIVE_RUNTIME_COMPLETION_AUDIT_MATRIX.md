...
Last refreshed from `main` evidence after `npm run mobile:one-event-runtime-status`, `npm run mobile:one-event-phase-audit`, and `npm run mobile:live-runtime-completion-audit` at `2026-07-14T14:17:50.712Z`, with current S23 close-position Max evidence in `docs/mobile/audits/cycle-S23CASHOUTMAX-close-position-max.md` and commit `98331055`.
