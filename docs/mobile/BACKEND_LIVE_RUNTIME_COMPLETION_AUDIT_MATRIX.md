...
Last refreshed from `main` evidence after `npm run mobile:internal-tester-readiness-gate` at `2026-07-14T09:38:06.583Z`, with survey reconciliation in `docs/mobile/audits/cycle-ZAV-live-runtime-survey-reconciliation.md` and durable result-poller run repair in `docs/mobile/audits/cycle-ZAX-durable-result-poller-run-repair.md`.
...
- Warm no-quota runtime loops: capability proven; current snapshot says both loops are not running right now and recommends starting the local tester runtime when needed.
