...
Last refreshed from `main` evidence after `npm run mobile:internal-tester-runtime -- -Action start -Force -ReplaceExternalExpo -StartSupervisor -StartResultPoller -RunResultIngestion -RunResultSettlement -WaitForReady -RuntimeOnlyArtifacts`, `npm run mobile:live-runtime-audit-gate`, and `npm run mobile:internal-tester-readiness-gate` at `2026-07-14T14:44:31.375Z`, with current S23 close-position Max evidence in `docs/mobile/audits/cycle-S23CASHOUTMAX-close-position-max.md` and latest committed baseline `0122f2ee`.
