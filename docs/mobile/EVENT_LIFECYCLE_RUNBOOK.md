...
36a. To inspect the active event's provider-result plus settlement-preflight evidence in one read-only operator report, run `npm run mobile:one-event-result-review-trail`. It spends no provider quota, does not execute settlement, and reports whether the operator should wait for `CLOSED` market status before execution.
