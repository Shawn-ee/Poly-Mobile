...
38. For a concise internal tester handoff snapshot, run `npm run mobile:internal-tester-operator-snapshot`. It reads backend health plus `/api/internal/live-runtime/status`, writes `docs/mobile/harness/odds-api-live-runtime/internal-tester-operator-snapshot.redacted.json`, reports the recommended next operator command, and does not call the provider or read provider keys.
...
44a. To refresh all live-runtime audit evidence in the correct order, run `npm run mobile:live-runtime-audit-gate`. It runs runtime status, phase audit, then completion audit, spends no provider quota, and prevents completion evidence from reading a stale phase artifact.
44b. To produce the current internal tester go/no-go summary, run `npm run mobile:internal-tester-readiness-gate`. It runs the ordered live-runtime audit gate, verifies source-aware sportsbook exchange readiness, then regenerates the operator snapshot and tester checklist from current backend status without provider quota. The summary is allowed to pass cached internal trading while reporting `liveOddsReady=false` or stopped supervisor/result-poller loops; use the recommended command in the summary when a warm loop session is needed.
...
43a. For ordinary manual tester sessions, prefer `npm run mobile:internal-tester-runtime:cached-start`. It starts/reuses backend and Expo, configures S23 ADB reverse, starts the cached supervisor and result poller, runs result-ingestion and settlement dry-run checks, and writes runtime summaries under `.runtime` so committed audit proof files do not churn while loops are running. Approved-settlement execution/wait mode is intentionally left to the dedicated settlement proof/operator commands.
...
- S23 visible proof: `docs/mobile/harness/cycle-ZCD-s23-live-event-proof/cycle-ZCD-S23-LIVE-odds-api-s23-visible-flow.json`. The current passing proof covers Argentina vs. England with close-position mode, owned-share Max, no Yes/No selector assertions, and the selected `Over 2.5` outcome identity.
...
- Trusted result settlement: `npm run mobile:one-event-result-settlement` reads trusted result JSON, maps the trusted score payload to the selected Total Goals 2.5 market outcome, previews settlement without mutation, and prints an exact result-digest confirmation phrase for execution. Current mobile/trading proof uses the normalized visible outcome label `Over 2.5`; older provider proof artifacts may still contain legacy `Over +2.5` labels and are not allowed to outrank quote-visible maker evidence.
...
- One-command onboarding: `npm run mobile:one-event-onboarding` blocks the old replay fixture, restores the cached one-event live-runtime event without provider quota, runs the readiness gate, runtime status, settlement readiness, provider-shaped result ingestion, trusted-result settlement dry run, and manual settlement dry run, with no unresolved P0 gaps when the local runtime is healthy.
- Internal tester operator snapshot: `npm run mobile:internal-tester-operator-snapshot` provides a small redacted command handoff from the local status route so testers can distinguish cached no-quota mode, optional live-provider refresh, and stopped/running local loops without treating proof artifacts as installed services.
