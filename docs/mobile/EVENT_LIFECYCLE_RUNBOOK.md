...
44b. To produce the current internal tester go/no-go summary, run `npm run mobile:internal-tester-readiness-gate`. It runs the ordered live-runtime audit gate, then regenerates the operator snapshot and tester checklist from current backend status without provider quota.
