...
| Continuous unattended daemon | P1 | Partial: local background process manager exists for start/status/stop, but it is not installed as an OS service or production process manager. |
| Always-on auto-close/suspend daemon | P1 | Partial: lifecycle scheduler runs inside the foreground/background supervisor while it is active. It is still not installed as an OS service. |
| Automatic official-result settlement | P1 | Manual/admin preview/resolve exists and non-mutating readiness is proven, but official soccer result ingestion and automatic resolve execution are still missing. |
