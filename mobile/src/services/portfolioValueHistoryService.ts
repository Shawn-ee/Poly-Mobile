...
export type LoadPortfolioValueHistoryInput = PortfolioValueHistoryInput & {
  api?: Pick<PolyApi, "getPortfolioValueHistory"> | null;
};
...
export const loadPortfolioValueHistory = async ({
  api,
  range,
  cash,
  positionsValue,
  pnl,
  now,
}: LoadPortfolioValueHistoryInput): Promise<PortfolioValueHistory> => {
  if (api) {
    try {
      const history = await api.getPortfolioValueHistory(range);
      if (history.range === range && Array.isArray(history.points)) {
        return history;
      }
    } catch {
      // Fall through to the deterministic offline shape when the route is unavailable.
    }
  }

  return deterministicPortfolioValueHistory({ range, cash, positionsValue, pnl, now });
};
