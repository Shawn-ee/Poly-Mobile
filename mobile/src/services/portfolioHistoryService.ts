...
export const portfolioHistoryToActivity = (history: Awaited<ReturnType<PolyApi["getPortfolioHistory"]>>["history"]): PortfolioActivity[] =>
  history.map((item) => {
    ...
    return {
      ...
      ...(item.market.eventTitle ? { eventTitle: item.market.eventTitle } : {}),
      ...(item.market.displayTitle ? { marketTitle: item.market.displayTitle } : {}),
      outcome: item.resolvedOutcomeName ?? "Resolved",
      amount: payout > 0 ? payout : item.netInvestedTokens,
      entryAmount: item.netInvestedTokens,
      realizedPnl: item.realizedPnLTokens,
      proceedsAmount: payout > 0 ? payout : item.netInvestedTokens,
...
export const canceledOrdersToActivity = (orders: PortfolioCanceledOrderItem[] = []): PortfolioActivity[] =>
  orders.map((order) => ({
    ...
    ...(order.market.displayTitle ? { marketTitle: order.market.displayTitle } : {}),
...
export const recentTradesToActivity = (trades: PortfolioRecentTradeItem[] = []): PortfolioActivity[] =>
  Array.from(
    ...
    return {
      ...
      ...(trade.market.eventTitle ? { eventTitle: trade.market.eventTitle } : {}),
      ...(trade.market.displayTitle ? { marketTitle: trade.market.displayTitle } : {}),
