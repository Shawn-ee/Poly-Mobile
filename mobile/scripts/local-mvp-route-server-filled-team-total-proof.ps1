...
try {
  ...
  try {
    ...
    cmd /c npx.cmd tsx scripts/seed_mobile_route_spread_counterparty.ts --eventSlug=$($eventProof.eventSlug) --marketGroupKey=team-totals --outcomeSide=over --askPrice=0.52 --askSize=60 --output=$counterpartyProofPath | Out-Null
    if ($LASTEXITCODE -ne 0) {
      throw "Route-backed team-total counterparty seed failed."
    }
