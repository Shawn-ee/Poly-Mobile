param(
  ...
  [string]$OutputDir = "docs\mobile\screenshots\cycle-OA-current-mvp-home-server-order",
  [string]$HierarchyOutputDir = "docs\mobile\harness\cycle-OA-current-mvp-home-server-order"
)
...
$proofUsername = "holiwyn-mobile-oa-$stamp"
$inspectionProofPath = "docs/mobile/harness/cycle-OA-current-mvp-home-server-order/cycle-OA-current-mvp-state-inspection.json"
...
try {
  ...
  try {
    cmd /c npx.cmd tsx scripts/inspect_mobile_mvp_current_state.ts --cycle=OA --output=$inspectionProofPath | Out-Null
    ...
    powershell -ExecutionPolicy Bypass -File "$PSScriptRoot\smoke-tablet.ps1" `
      -LocalMvpHomeRouteServerOrderFlow `
      -Device $Device `
      -Port $Port `
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "S23 Home route-backed server order smoke failed."
    }

    $summaryPath = Join-Path $repoRoot (Join-Path $HierarchyOutputDir "cycle-OA-current-mvp-home-server-order-wrapper.json")
    ...
    [ordered]@{
      ...
      androidProofPath = Join-Path $HierarchyOutputDir "cycle-OA-current-mvp-home-server-order-proof.json"
      orderMode = "server"
