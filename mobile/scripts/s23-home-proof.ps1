param(
  ...
  [int]$Port = 8182,
  [string]$ExpoHost = "",
  [string]$OutputDir = "docs\mobile\screenshots\cycle-VW-home-copy-contract-cleanup",
  [string]$HierarchyOutputDir = "docs\mobile\harness\cycle-VW-home-copy-contract-cleanup"
)
...
function Save-Screenshot {
  ...
  Write-Host "S23 Home proof screenshot: $local"
}
...
function Save-UiHierarchy {
  ...
  & $adb -s $Device pull $remote $local | Out-Null
  Write-Host "S23 Home proof hierarchy: $local"
  return $local
}
...
function Dismiss-ExpoDeveloperMenuIfPresent {
  $path = Save-UiHierarchy -Name "cycle-VW-home-expo-menu.xml"
  $hierarchy = Get-Content -Raw -Path $path
...
$expoLog = Join-Path $MobileRoot "mobile-s23-home-proof-expo.log"
$expoErrorLog = Join-Path $MobileRoot "mobile-s23-home-proof-expo-error.log"
...
try {
  npm run typecheck
  Write-Host "S23 Home proof target: $Device"
  ...
  Start-DeepLink -Url "exp://${resolvedExpoHost}:$Port/--/?forceResetState=1"
  Start-Sleep -Seconds 10
  Dismiss-ExpoDeveloperMenuIfPresent
  Start-Sleep -Seconds 3

  $homePath = Wait-HierarchyContains -Name "cycle-VW-home.xml" -Expected @(
    "Holiwyn",
    "home-world-cup-games-focus",
    "World Cup",
    "Matches"
  )
  Save-Screenshot -Name "cycle-VW-home.png"
  Assert-HierarchyContains -Path $homePath -Expected @(
    "Holiwyn",
    "home-world-cup-games-focus",
    "World Cup",
    "Matches",
    "matches",
    "live",
    "home-secondary-markets-hidden-local-mvp",
    "holiwyn-home-tab"
  )
  Assert-HierarchyDoesNotContain -Path $homePath -Unexpected @(
    "Search World Cup markets",
    "Trending",
    "game predictions",
    "world-cup-futures-tab",
    "home-filter-all",
    "home-filter-live",
    "home-filter-today",
