...
try {
  ...
  if ($Deep) {
    ...
    if ($ServerOrderFailure) {
      ...
      Assert-HierarchyContains -Path $serverTicketHierarchy -Expected @("Trading mode: Server mode", "ticket-market-depth", "Best bid", "Best ask", "Spread", "Fake balance", "10,000 USDT", "Estimated cost", "Est. fee", "0 USDT", "ticket-slippage", "Slippage", "1%", "Est. shares", "Avg price", "Place buy order")
    ...
    if ($LiveTicket -or $LiveOrder -or $LiveOrderClose -or $LivePortfolioBadge -or $LivePortfolioBadgeDeep) {
      ...
      $liveTicketHierarchy = ""
      $liveTicketExpected = @("France", "France vs. Argentina", "Trading mode: Fake-token mock", "ticket-market-depth", "Best bid", "Best ask", "Spread", "Live World Cup", "ticket-live-clock", "Live - 63'", "Fake balance", "10,000 USDT", "Estimated cost", "Est. fee", "0 USDT", "ticket-slippage", "Slippage", "1%", "Est. shares", "Avg price", "Place buy order")
    ...
    if ($FutureListTrade) {
      ...
      Assert-HierarchyContains -Path $futureListTicketHierarchy -Expected @("World Cup winner", "France", "Trading mode: Fake-token mock", "ticket-market-depth", "Best bid", "Best ask", "Spread", "Fake balance", "10,000 USDT", "Est. fee", "0 USDT", "ticket-slippage", "Slippage", "1%", "Est. shares", "Avg price", "Place buy order")
      return
    }
    ...
    if ($FutureListOrder) {
      ...
      Assert-HierarchyContains -Path $futureListOrderTicketHierarchy -Expected @("World Cup winner", "France", "ticket-trading-mode", "Trading mode: Fake-token mock", "ticket-market-depth", "Best bid", "Best ask", "Spread", "ticket-estimated-fee", "Est. fee", "0 USDT", "ticket-slippage", "Slippage", "1%", "Fake balance", "10,000 USDT", "Place buy order")
    ...
    if (-not ($EventDetailTrade -or $EventDetailSummary -or $EventDetailMarketOutcomeCount)) {
      Invoke-TapHierarchyNode -Path $homeHierarchy -Identifier "event-card-mexico-ecuador"
      Start-Sleep -Seconds 1
    }
