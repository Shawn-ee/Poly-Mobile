...
try {
  ...
  if ($Deep) {
    ...
    if ($LiveTicket) {
      ...
      Assert-HierarchyContains -Path $liveTicketHierarchy -Expected @("France", "France vs. Argentina", "Fake balance", "10,000 USDT", "Estimated cost", "Est. shares", "Avg price", "Place buy order")
      return
    }
    ...
    if (-not ($EventDetailSummary -or $EventDetailMarketOutcomeCount)) {
      Invoke-TapHierarchyNode -Path $homeHierarchy -Identifier "event-card-mexico-ecuador"
      Start-Sleep -Seconds 1
    }
