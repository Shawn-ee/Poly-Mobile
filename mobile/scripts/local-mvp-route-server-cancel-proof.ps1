param(
  ...
  [string]$OutputDir = "docs\mobile\screenshots\cycle-EW-local-mvp-route-server-cancel-flow",
  [string]$HierarchyOutputDir = "docs\mobile\harness\cycle-EW-local-mvp-route-server-cancel-flow"
)
...
$proofUsername = "holiwyn-mobile-ew-$stamp"
$eventProofPath = "docs/mobile/harness/cycle-EW-local-mvp-route-server-cancel-flow/cycle-EW-route-backed-retail-event.json"
...
try {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Tablet route-backed server cancel smoke failed."
    }

    $summaryPath = Join-Path $repoRoot (Join-Path $HierarchyOutputDir "cycle-EW-local-mvp-route-server-cancel-flow-wrapper.json")
    ...
    [ordered]@{
      ...
      tabletProofPath = Join-Path $HierarchyOutputDir "cycle-EW-local-mvp-route-server-cancel-flow-proof.json"
      orderMode = "server"
