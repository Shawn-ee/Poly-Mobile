param(
  ...
  [string]$OutputDir = "docs\mobile\screenshots\cycle-EV-local-mvp-route-server-order-flow",
  [string]$HierarchyOutputDir = "docs\mobile\harness\cycle-EV-local-mvp-route-server-order-flow"
)
...
$proofUsername = "holiwyn-mobile-ev-$stamp"
$eventProofPath = "docs/mobile/harness/cycle-EV-local-mvp-route-server-order-flow/cycle-EV-route-backed-retail-event.json"
...
try {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Tablet route-backed server order smoke failed."
    }

    $summaryPath = Join-Path $repoRoot (Join-Path $HierarchyOutputDir "cycle-EV-local-mvp-route-server-order-flow-wrapper.json")
    ...
    [ordered]@{
      ...
      tabletProofPath = Join-Path $HierarchyOutputDir "cycle-EV-local-mvp-route-server-order-flow-proof.json"
      orderMode = "server"
