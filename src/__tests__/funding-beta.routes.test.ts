...
jest.mock("@/lib/deposits/polygonDeposits", () => ({
  listUserPolygonDeposits: (...args: unknown[]) => listUserPolygonDeposits(...args),
}));
...
describe("funding beta routes", () => {
  ...
  test("deposit history route blocks non-allowlisted users before listing deposits", async () => {
    const { FundingAccessError } = await import("@/lib/fundingBeta");
    ...
    expect(res.status).toBe(403);
    expect(body.code).toBe("FUNDING_NOT_ALLOWLISTED");
    expect(listUserPolygonDeposits).not.toHaveBeenCalled();
  });

  test("deposit history route returns public deposit fields only", async () => {
    const { GET } = await import("@/app/api/deposits/route");
    resolveAuthenticatedUser.mockResolvedValue({
      user: { id: "u1", email: "internal@example.com", username: "internal", isAdmin: false },
      reason: null,
    ...
    requireInternalFundingUser.mockReturnValue({ id: "u1" });
    listUserPolygonDeposits.mockResolvedValue([
      {
        id: "dep-1",
        amount: { toFixed: () => "12.340000" },
        txHash: "0xtransfer",
        fromAddress: "0x2222222222222222222222222222222222222222",
        toAddress: "0x1111111111111111111111111111111111111111",
        status: "CREDITED",
        confirmations: 25,
        detectedAt: new Date("2026-06-19T00:00:00.000Z"),
        creditedAt: new Date("2026-06-19T00:01:00.000Z"),
    ...
    expect(body.items[0].privateKey).toBeUndefined();
    expect(body.items[0].encryptedPrivateKey).toBeUndefined();
    expect(body.items[0].mnemonic).toBeUndefined();
    expect(body.items[0].seed).toBeUndefined();
    expect(JSON.stringify(body)).not.toContain("secret");
  });
