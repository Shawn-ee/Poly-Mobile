...
describe("The Odds API single-event temporary provider", () => {
  ...
  it("exposes a redacted no-quota live odds refresh preflight", () => {
    ...
    expect(source).toContain("providerSecretFilePresent");
    expect(source).toContain("providerKeySource");
    expect(source).toContain("local-secret-file");
    expect(source).toContain("[\"THE\", \"ODDS\", \"API\", \"KEY\"].join(\"_\")");
    expect(source).toContain("providerKeyValuePrinted: false");
    expect(source).toContain("commandLineContainsSecret: false");
    expect(source).toContain("providerSecretFileCheckedOnlyForPresence: true");
  ...
  it("gates live-runtime audits on managed S23 server-backed startup", () => {
    ...
    expect(phaseAuditScript()).toContain("manager-owned-expo-start-summary.redacted.json");
