        ...
        items: [
          {
            ...
            operatorExecutionPlan: {
              ...
              command: {
                ...
                args: ["--dry-run"],
                commandRedacted: "npm run mobile:one-event-settlement-preflight",
...
describe("live runtime status service", () => {
  ...
  test("returns ready only when audits pass and proof artifacts are fresh", async () => {
    ...
    expect(status.productionReadinessBoundary).toMatchObject({
      ...
      cachedTesterReadyRightNow: false,
    ...
    expect(status.operatorControlBoundary).toMatchObject({
      ...
      productionAuthRequirements: {
        ...
        requiredRoutes: expect.arrayContaining([
          ...
          expect.objectContaining({
            ...
            requiresClosedMarket: true,
            requiresExactConfirmation: true,
            implementationStatus: "implemented_guarded_exact_confirmation_local_execution",
            dryRunOnly: false,
            exactConfirmationExecutionSupported: true,
            executeRequiresApproval: true,
            executeRequiresTwoPersonOrAdminPolicy: true,
            exactConfirmationStored: false,
            activeSettlementExecution: false,
          }),
        ]),
      ...
      localControls: {
        ...
        settlementExecutionRoute: {
          ...
          available: true,
          mutatesState: true,
          mutationScope: "dry_run_request_or_exact_confirmed_closed_market_execution",
          dryRunOnly: false,
          exactConfirmationExecutionSupported: true,
          executeRequiresClosedMarket: true,
          executeRequiresApproval: true,
          executeRequiresExactConfirmation: true,
          operatorAuditEventRecorded: true,
    ...
    expect(status.serviceOwnership).toMatchObject({
      ...
      unattendedReadiness: {
        checked: true,
        classification: "local_user_startup_fallback_ready_scheduled_task_permission_blocked",
        localInternalTesterReady: true,
        installedProductionServiceReady: false,
        productionDaemonInstalled: false,
        foregroundLoopsProven: true,
        startupFallbackProven: true,
        startupLauncherInstalledNow: false,
        noPersistentStartupLauncherAfterProof: true,
        scheduledTaskPlanProven: true,
        scheduledTaskInstallBlockedByWindowsPermission: true,
        scheduledTaskInstalledNow: false,
        noPersistentScheduledTaskAfterProof: true,
        noProviderQuotaByDefault: true,
        recommendedInternalMode: "user_startup_fallback_or_manual_cached_runtime",
        nextOperatorAction: "use_cached_start_for_testing_or_install_user_startup_launcher_if_desired",
        p0: [],
        p1: expect.arrayContaining([
          "installed_production_service_missing",
          "scheduled_task_registration_permission_blocked",
        ]),
      },
      p0: [],
      p1: expect.arrayContaining([
