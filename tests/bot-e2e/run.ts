...
import { BOT_E2E_FIXTURE_PATH, type BotE2EFixture } from "./shared";
...
const readFixture = async () => {
  ...
  return JSON.parse(raw) as BotE2EFixture;
};
...
const waitForApp = async (baseUrl: string, timeoutMs = 60_000) => {
...
const parseSseFrame = (buffer: string) => {
  ...
  const frame: { id: string | null; event: string | null; data: string[] } = {
    id: null,
    event: null,
    data: [],
  };
...
const readFirstSseEvent = async (response: Response, timeoutMs = 10_000) => {
  ...
  return Promise.race([
    timedRead(),
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`Timed out waiting for SSE event after ${timeoutMs}ms.`)), timeoutMs)
    ),
  ]);
};
...
async function main() {
  ...
  const runScenario = async (name: string, fn: () => Promise<void>) => {
  ...
  await runScenario("2. Read market, quote, fills, positions, ledger, and balance", async () => {
    ...
    assert.ok(markets.body.markets.some((market: { id: string }) => market.id === fixture.marketId));

    const quote = await fetchJson(`${fixture.baseUrl}/api/markets/${fixture.marketId}/quote`);
    assert.equal(quote.response.status, 200);
    const yesQuote = quote.body.quotes.find((item: { outcomeId: string }) => item.outcomeId === fixture.yesOutcomeId);
    assert.ok(yesQuote);
  ...
  await runScenario("3. Place a limit buy order successfully", async () => {
    ...
    assert.ok(list.body.items.some((item: { id: string }) => item.id === mainOrderId));
